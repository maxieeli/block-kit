...
import { Job } from '@maxiee/block_store';
...
export class Clipboard {
  ...
  async writeToClipboard(
    ...
    const html = `<div data-blockkit-snapshot=${snapshot}>${innerHTML}</div>`;
  ...
  readFromClipboard(clipboardData: DataTransfer) {
    ...
    const dom = doc.querySelector<HTMLDivElement>('[data-blockkit-snapshot]');
    assertExists(dom);
    const json = JSON.parse(
      lz.decompressFromEncodedURIComponent(
        dom.dataset.blockkitSnapshot as string
      )
    );
    return json;
  }
