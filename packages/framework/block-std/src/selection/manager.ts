...
import type { StackItem } from '@maxiee/block_store';
...
export class SelectionManager {
  ...
  constructor(public std: BlockKit.Std) {
  ...
  create<T extends BlockKit.SelectionType>(
    type: T,
    ...args: ConstructorParameters<BlockKit.Selection[T]>
  ): BlockKit.SelectionInstance[T] {
    const ctor = this._selectionConstructors[type];
    if (!ctor) {
      throw new Error(`Unknown selection type: ${type}`);
    }
    return new ctor(...args) as BlockKit.SelectionInstance[T];
  }
  ...
  find<T extends BlockKit.SelectionType>(type: T) {
    return this.value.find((sel): sel is BlockKit.SelectionInstance[T] =>
      sel.is(type)
    );
  }

  filter<T extends BlockKit.SelectionType>(type: T) {
    return this.value.filter((sel): sel is BlockKit.SelectionInstance[T] =>
      sel.is(type)
    );
  }
