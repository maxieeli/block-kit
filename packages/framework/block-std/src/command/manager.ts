...
export class CommandManager {
  ...
  private _createChain = (
    methods: Record<BlockKit.CommandName, unknown>,
    _cmds: Command[]
  ): Chain => {
    ...
    return {
      ...
      run: function (this: Chain) {
        ...
        try {
          ...
          ctx = runCmds(ctx as BlockKit.CommandContext, [
            ...cmds,
            (_, next) => {
              success = true;
              next();
  ...
  add<N extends BlockKit.CommandName>(
    name: N,
    command: BlockKit.Commands[N]
  ): CommandManager;
  ...
  exec<K extends keyof BlockKit.Commands>(
    command: K,
    ...args: IfAllKeysOptional<
      Omit<InDataOfCommand<BlockKit.Commands[K]>, keyof InitCommandCtx>,
      [
        inData: void | Omit<
          InDataOfCommand<BlockKit.Commands[K]>,
...
function runCmds(ctx: BlockKit.CommandContext, [cmd, ...rest]: Command[]) {
  let _ctx = ctx;
  if (cmd) {
    cmd(ctx, data => {
