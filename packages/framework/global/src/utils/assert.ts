...
import { BlockKitError } from '../exceptions/index.js';
...
export function assertExists<T>(
  ...
  if (val === null || val === undefined) {
    ...
    throw new BlockKitError(ErrorCode.ValueNotExists, message);
  }
}
...
export function assertEquals<T extends Allowed, U extends T>(
  ...
  if (!isEqual(val, expected)) {
    throw new BlockKitError(ErrorCode.ValueNotEqual, message);
  }
}
...
export function assertInstanceOf<T>(
  ...
  if (!(val instanceof expected)) {
    throw new BlockKitError(ErrorCode.ValueNotInstanceOf, message);
  }
}
