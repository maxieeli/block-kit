import { assertExists, sha } from '@maxiee/block_global/utils';
import type { DeltaInsert } from '@maxiee/block_inline/types';
import type {
...
import type { WorkbenchTextAttributes } from '../inline/presets/workbench-inline-specs.js';
...
export class MarkdownAdapter extends BaseAdapter<Markdown> {
    ...
    walker.setEnter(async (o, context) => {
      ...
      switch (o.node.flavour) {
        ...
        case 'workbench:list': {
          ...
          if (
            context.getNodeContext('workbench:list:parent') === o.parent &&
            ...
            context
              ...
              .setNodeContext('workbench:list:parent', o.parent)
              .openNode(
                {
                  type: 'listItem',
                  checked:
        ...
        case 'workbench:embed-synced-doc': {
        ...
        case 'workbench:embed-loom':
        case 'workbench:embed-github':
        case 'workbench:embed-youtube':
        case 'workbench:embed-figma':
        case 'workbench:bookmark': {
          // Parse as link
          if (
    ...
    walker.setLeave((o, context) => {
      ...
      switch (o.node.flavour) {
        case 'workbench:paragraph': {
          context.setGlobalContext(
            'workbench:paragraph:depth',
            (context.getGlobalContext('workbench:paragraph:depth') as number) - 1
          );
          break;
        }
        case 'workbench:list': {
          if (
            context.getPreviousNodeContext('workbench:list:parent') === o.parent &&
            ...
            context.closeNode();
            if (o.next?.flavour !== 'workbench:list') {
    ...
    walker.setEnter(async (o, context) => {
      switch (o.node.type) {
        ...
        case 'table': {
          ...
          context.setNodeContext('workbench:table:rowid', Object.keys(cells));
          context.skipChildren(1);
          break;
        }
        case 'tableRow': {
          context
            .openNode({
              ...
              id:
                (
                  context.getNodeContext('workbench:table:rowid') as Array<string>
                ).shift() ?? nanoid(),
              flavour: 'workbench:paragraph',
              props: {
                text: {
  ...
  private _deltaToMdAST(
    deltas: DeltaInsert<WorkbenchTextAttributes>[],
    depth = 0
  ) {
    if (depth > 0) {
