...
export class FileDropManager {
  ...
  get targetModel(): BlockModel | null {
    ...
    if (!targetModel && isInsidePageEditor(this.editorHost)) {
      ...
      if (!lastNote || !matchFlavours(lastNote, ['workbench:note'])) {
        const newNoteId = this.doc.addBlock('workbench:note', {}, rootModel.id);
  ...
  onDragOver = (event: DragEvent) => {
    ...
    if (element) {
      ...
      if (!matchFlavours(parent, ['workbench:surface'])) {
