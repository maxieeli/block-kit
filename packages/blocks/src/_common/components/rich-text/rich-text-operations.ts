...
import { type BlockModel, Text } from '@maxiee/block_store';
...
export function handleBlockEndEnter(
  ...
  const getProps = ():
    | [
        'workbench:list',
        BlockKit.ModelProps<BlockKit.BlockModels['workbench:list']>,
      ]
    | [
        'workbench:paragraph',
        BlockKit.ModelProps<BlockKit.BlockModels['workbench:paragraph']>,
      ] => {
    const shouldInheritFlavour = matchFlavours(model, ['workbench:list']);
    if (shouldInheritFlavour) {
      return [model.flavour, { type: model.type }];
    }
    return ['workbench:paragraph', { type: 'text' }];
  };
  const [flavour, blockProps] = getProps();

  if (isInsideBlockByFlavour(doc, model, 'workbench:database')) {
    doc.captureSync();
    ...
    if (index === childrenLength - 1 && model.text?.yText.length === 0) {
      ...
      if (nextModel && matchFlavours(nextModel, ['workbench:paragraph'])) {
  ...
  if (matchFlavours(model, ['workbench:list']) && model.type === 'numbered') {
    let next = nextSibling;
    while (
      next &&
      matchFlavours(next, ['workbench:list']) &&
      model.type === 'numbered'
    ) {
...
export function handleIndent(
  ...
  nextSiblings
    .filter(
      sibling =>
        matchFlavours(sibling, ['workbench:list']) && sibling.type === 'numbered'
    )
    .forEach(sibling => {
  ...
  if (
    matchFlavours(previousSibling, ['workbench:list']) &&
...
export function handleUnindent(
  ...
  model.children.forEach(child => {
    if (matchFlavours(child, ['workbench:list']) && child.type === 'numbered') {
...
export function handleMultiBlockOutdent(
  ...
  for (let i = 0; i < models.length; i++) {
    ...
    if (firstParent && !matchFlavours(firstParent, ['workbench:note'])) {
      firstOutdentIndex = i;
      break;
    }
  }
...
export function handleRemoveAllIndent(
  ...
  while (parent && !matchFlavours(parent, ['workbench:note'])) {
...
export function handleRemoveAllIndentForMultiBlocks(
  ...
  for (let i = models.length - 1; i >= 0; i--) {
    ...
    const parent = doc.getParent(model);
    assertExists(parent);
    if (!matchFlavours(parent, ['workbench:note'])) {
...
function handleCodeBlockForwardDelete(model: ExtendedModel) {
  if (!matchFlavours(model, ['workbench:code'])) return false;
  return true;
}

function handleDatabaseBlockForwardDelete(model: ExtendedModel) {
  const doc = model.doc;
  if (!isInsideBlockByFlavour(doc, model, 'workbench:database')) return false;
...
function handleListBlockBackspace(
  ...
  const doc = model.doc;
  if (!matchFlavours(model, ['workbench:list'])) return false;
...
function handleParagraphOrListSibling(
  ...
  const doc = model.doc;
  if (!matchFlavours(previousSibling, ['workbench:paragraph', 'workbench:list']))
    return false;
...
function handleEmbedDividerCodeSibling(
  ...
  const doc = model.doc;
  if (matchFlavours(previousSibling, ['workbench:divider'])) {
...
function handleNoPreviousSibling(editorHost: EditorHost, model: ExtendedModel) {
  ...
  if (!titleElement) {
    if (
      matchFlavours(parent, ['workbench:edgeless-text']) ||
...
function handleParagraphDeleteActions(
  ...
  if (!previousSibling) {
    ...
    matchFlavours(previousSibling, ['workbench:paragraph', 'workbench:list'])
  ) {
    ...
    return true;
  } else if (matchFlavours(previousSibling, ['workbench:edgeless-text'])) {
    return true;
  }

  // TODO handle in block service
  if (matchFlavours(parent, ['workbench:database'])) {
    doc.deleteBlock(model);
    focusBlockByModel(editorHost, previousSibling);
    return true;
  } else if (matchFlavours(parent, ['workbench:note'])) {
    return (
...
function handleParagraphBlockBackspace(
  ...
  const doc = model.doc;
  if (!matchFlavours(model, ['workbench:paragraph'])) return false;
...
function handleParagraphBlockForwardDelete(
  ...
  function handleParagraphOrList(
    ...
    function handleParagraphOrListSibling(
      ...
      if (
        nextSibling &&
        matchFlavours(nextSibling, ['workbench:paragraph', 'workbench:list'])
      ) {
        model.text?.join(nextSibling.text as Text);
        if (nextSibling.children) {
          const parent = doc.getParent(nextSibling);
        ...
        if (
          !nextBlock ||
          !matchFlavours(nextBlock, ['workbench:paragraph', 'workbench:list'])
        )
          return false;
        model.text?.join(nextBlock.text as Text);
        if (nextBlock.children) {
    ...
    function handleParagraphOrListChild(
      ...
      if (
        !firstChild ||
        !matchFlavours(firstChild, ['workbench:paragraph', 'workbench:list'])
      ) {
        return false;
      }
      const grandChildren = firstChild.children;
  ...
  function handleEmbedDividerCode(
    ...
    function handleEmbedDividerCodeSibling(nextSibling: ExtendedModel | null) {
      if (matchFlavours(nextSibling, ['workbench:divider'])) {
      ...
      if (
        !nextSibling ||
        !matchFlavours(nextSibling, ['workbench:image', 'workbench:code'])
      )
        return false;
  ...
  if (matchFlavours(parent, ['workbench:database'])) {
    ...
    const ignoreForwardDeleteFlavourList: BlockKit.Flavour[] = [
      'workbench:database',
      'workbench:image',
      'workbench:code',
      'workbench:attachment',
