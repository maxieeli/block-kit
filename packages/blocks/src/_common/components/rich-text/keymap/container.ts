import type { BlockElement, UIEventStateContext } from '@maxiee/block-std';
...
export const bindContainerHotkey = (blockElement: BlockElement) => {
  ...
  blockElement.bindHotKey({
    ...
    'Mod-Backspace': ctx => {
      ...
      const rootElement = blockElement.closest<RootBlockComponent>(
        'workbench-page-root,workbench-edgeless-root'
      );
    ...
    Backspace: ctx => {
      ...
      if (matchFlavours(blockElement.model, ['workbench:code'])) {
        const inlineRange = inlineEditor.getInlineRange();
        assertExists(inlineRange);
    ...
    blockElement.bindHotKey({
      [pair.left]: ctx => {
        ...
        if (
          ...
          !matchFlavours(blockElement.model, ['workbench:code'])
        )
          return;
  ...
  bracketPairs.forEach(pair => {
    blockElement.bindHotKey({
      [pair.right]: ctx => {
        if (!matchFlavours(blockElement.model, ['workbench:code'])) return;
        const inlineEditor = _getInlineEditor();
        const inlineRange = inlineEditor.getInlineRange();
        assertExists(inlineRange);
        const left = inlineEditor.yText.toString()[inlineRange.index - 1];
