...
import { assertExists, isEqual } from '@maxiee/block_global/utils';
import {
...
export function tryConvertBlock(
  ...
  if (codeMatch) {
    if (
      model.flavour === 'workbench:paragraph' &&
    ...
    if (model.text && model.text.length > prefixText.length) {
      ...
      doc.addBlock('workbench:paragraph', { text }, parent, index + 1);
