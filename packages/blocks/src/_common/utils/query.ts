...
import type { InlineEditor } from '@maxiee/block_inline';
...
// margin-top: calc(var(--workbench-paragraph-space) + 24px);
 ...
 * ```
 *
 * NOTE: this method will skip the `workbench:note` block
 */
export function getNextBlock(
  ...
  while (currentBlock) {
    ...
    if (nextSibling) {
      // Assert nextSibling is not possible to be `workbench:page`
      if (matchFlavours(nextSibling, ['workbench:note'])) {
...
export function getPreviousBlock(
  ...
  const getPrev = (model: BlockModel) => {
    ...
    if (
      ...
      matchFlavours(parent, ['workbench:note'])
    ) {
      return null;
    }
  ...
  const iterate: (model: BlockModel) => BlockModel | null = (
    ...
    if (prev) {
      if (prev.role === 'content' && !matchFlavours(prev, ['workbench:frame'])) {
        return prev;
      } else {
        return iterate(prev);
      }
...
export function blockElementGetter(model: BlockModel, view: ViewStore) {
  if (matchFlavours(model, ['workbench:image', 'workbench:frame'])) {
    let current: BlockModel | null = model;
    const path: string[] = [];
    while (current) {
      // Top level image render under root block not surface block
      if (!matchFlavours(current, ['workbench:surface'])) {
        path.unshift(current.id);
      }
      current = current.doc.getParent(current);
    }
...
export function getViewportElement(editorHost: EditorHost) {
  ...
  if (!rootElement || rootElement.closest('workbench-page-root') !== rootElement) {
...
export function getInlineEditorByModel(
  ...
  if (matchFlavours(model, ['workbench:database'])) {
...
export function getDocTitleByEditorHost(
  ...
  const docViewport = editorHost.closest('.workbench-page-viewport');
...
function isDocPage({ tagName }: Element) {
  return tagName === 'WORKBENCH-PAGE-ROOT';
}
...
export function isEdgelessPage(
  ...
  return element.tagName === 'WORKBENCH-EDGELESS-ROOT';
}
...
function isImage({ tagName }: Element) {
  return tagName === 'WORKBENCH-IMAGE';
}

function isNote({ tagName }: Element) {
  return tagName === 'WORKBENCH-NOTE';
}

function isSurface({ tagName }: Element) {
  return tagName === 'WORKBENCH-SURFACE';
}

function isDatabase({ tagName }: Element) {
  return tagName === 'WORKBENCH-DATABASE-TABLE' || tagName === 'WORKBENCH-DATABASE';
}
...
export function getClosestBlockElementByPoint(
  ...
  if (element) {
    ...
    if (isDatabase(element)) {
      ...
      childBounds = element
        .querySelector('.workbench-block-children-container')
 ...
 * In Chrome/Safari, `document.elementsFromPoint` does not include `workbench-image`.
 */
...
function getDatabaseBlockTableElement(element: Element) {
  return element.querySelector('.workbench-database-block-table');
}
...
function getDatabaseBlockColumnHeaderElement(element: Element) {
  return element.querySelector('.workbench-database-column-header');
}
...
function getDatabaseBlockRowsElement(element: Element) {
  return element.querySelector('.workbench-database-block-rows');
}
...
export function getDropRectByPoint(
  ...
  const isDatabase = matchFlavours(model, ['workbench:database']);
  ...
  if (isDatabase) {
    ...
    if (model.isEmpty()) {
      ...
      for (; i < len; i++) {
        ...
        if (e.classList.contains('workbench-database-block-row-cell-content')) {
          result.rect = getCellRect(e, bounds);
          return result;
        }

        if (e.classList.contains('workbench-database-block-row')) {
    ...
    if (parent?.classList.contains('workbench-database-block-row-cell-content')) {
      result.flag = DropFlags.Database;
...
function getCellRect(element: Element, bounds?: DOMRect) {
  if (!bounds) {
    const table = element.closest('.workbench-database-block-table');
    assertExists(table);
    bounds = table.getBoundingClientRect();
  }
  // workbench-database-block-row-cell
  const col = element.parentElement;
  assertExists(col);
  // workbench-database-block-row
