...
import type { InlineRange, VLine } from '@maxiee/block_inline';
...
export function asyncFocusRichText(
  ...
  assertExists(model);
  if (matchFlavours(model, ['workbench:divider'])) return;
...
function setNewTop(y: number, editableContainer: Element, zoom = 1) {
  ...
  const scrollContainer = editableContainer.closest('.workbench-page-viewport');
...
export function focusBlockByModel(
  ...
  if (matchFlavours(model, ['workbench:note', 'workbench:page'])) {
