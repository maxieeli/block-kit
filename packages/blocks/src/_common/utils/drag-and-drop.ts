...
export function calcDropTarget(
  ...
  const schema = model.doc.getSchemaByFlavour('workbench:database');
  assertExists(schema);
  ...
  if (!shouldAppendToDatabase && !matchFlavours(model, ['workbench:database'])) {
    const databaseBlockElement = element.closest('workbench-database');
