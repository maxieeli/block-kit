...
import { PathFinder } from '@maxiee/block-std';
...
export const embedNoteContentStyles = css`
  .workbench-embed-doc-content-note-blocks workbench-divider,
  .workbench-embed-doc-content-note-blocks workbench-divider > * {
  ...
  .workbench-embed-doc-content-note-blocks workbench-paragraph,
  .workbench-embed-doc-content-note-blocks workbench-list {
  ...
  .workbench-embed-doc-content-note-blocks workbench-list .workbench-list-block__prefix {
    height: 20px;
  }
  .workbench-embed-doc-content-note-blocks workbench-paragraph .quote {
    padding-left: 15px;
    padding-top: 8px;
    padding-bottom: 8px;
  }
  .workbench-embed-doc-content-note-blocks workbench-paragraph:has(.h1),
  .workbench-embed-doc-content-note-blocks workbench-paragraph:has(.h2),
  .workbench-embed-doc-content-note-blocks workbench-paragraph:has(.h3),
  .workbench-embed-doc-content-note-blocks workbench-paragraph:has(.h4),
  .workbench-embed-doc-content-note-blocks workbench-paragraph:has(.h5),
  .workbench-embed-doc-content-note-blocks workbench-paragraph:has(.h6) {
  ...
  .workbench-embed-linked-doc-block.horizontal {
    workbench-paragraph,
    workbench-list {
    ...
    workbench-paragraph .quote {
      padding-top: 4px;
      padding-bottom: 4px;
      height: 28px;
    }
    workbench-paragraph .quote::after {
      height: 20px;
      margin-top: 4px !important;
      margin-bottom: 4px !important;
    }
...
export function promptDocTitle(host: EditorHost, autofill?: string) {
  const notification =
    host.std.spec.getService('workbench:page').notificationService;
...
export function getTitleFromSelectedModels(selectedModels: BlockModel[]) {
  ...
  if (
    matchFlavours(firstBlock, ['workbench:paragraph']) &&
...
export function getNotesFromDoc(doc: Doc) {
  const notes = doc.root?.children.filter(
    child =>
      matchFlavours(child, ['workbench:note']) &&
...
export function isEmptyNote(note: BlockModel) {
  return note.children.every(block => {
    return (
      block.flavour === 'workbench:paragraph' &&
...
function filterTextModel(model: BlockModel) {
  ...
  if (!matchFlavours(model, ['workbench:paragraph', 'workbench:list'])) {
    return false;
  }
...
async function renderNoteContent(
  ...
  noteBlocksContainer.classList.add('workbench-embed-doc-content-note-blocks');
...
function renderSurfaceRef(
  ...
  const surfaceRefService = card.std.spec.getService('workbench:surface-ref');
...
async function renderPageAbstract(
  ...
  const target = notes.flatMap(note =>
    note.children.filter(child => matchFlavours(child, ['workbench:image']))
  )[0];
...
export function addBlocksToDoc(
  ...
  const newModelId = targetDoc.addBlock(
    model.flavour as BlockKit.Flavour,
    blockProps,
    parentId
  );
...
export function convertSelectedBlocksToLinkedDoc(
  ...
  doc.addSiblingBlocks(
    ...
    [
      {
        flavour: 'workbench:embed-linked-doc',
        pageId: linkedDoc.id,
      },
    ],
    'before'
...
export function createLinkedDocFromBlocks(
  ...
  linkedDoc.load(() => {
    const rootId = linkedDoc.addBlock('workbench:page', {
      title: new doc.Text(docTitle),
    });
    linkedDoc.addBlock('workbench:surface', {}, rootId);
    const noteId = linkedDoc.addBlock('workbench:note', {}, rootId);
...
export function createLinkedDocFromEdgelessElements(
  ...
  linkedDoc.load(() => {
    ...
    const surfaceId = linkedDoc.addBlock('workbench:surface', {}, rootId);
    ...
    sortedElements.forEach(model => {
      ...
      if (model instanceof EdgelessBlockModel) {
        ...
        if (isNoteBlock(model)) {
          newId = linkedDoc.addBlock('workbench:note', blockProps, rootId);
  ...
  const pageService = host.spec.getService('workbench:page');
