...
export class ExportManager {
    ...
    const promise = new Promise((resolve, reject) => {
      ...
      const checkReactRender = setInterval(() => {
        ...
        const rootElement = this.doc.root
          ...
          : null;
        const imageCard = rootElement?.querySelector('workbench-image-block-card');
        const isReady =
  ...
  private async _html2canvas(
    ...
    const html2canvasOption = {
      ...
      onclone: async (documentClone: Document, element: HTMLElement) => {
        ...
        const layer = element.classList.contains('.workbench-edgeless-layer')
          ? element
          : null;
    ...
    const html2canvasOption = {
      ignoreElements: function (element: Element) {
        if (
          ...
          // the close and expand buttons in workbench-doc-meta-data is not needed to be showed
          return true;
        } else {
          return false;
        }
  ...
  async edgelessToCanvas(
    ...
    surfaces?: BlockKit.SurfaceElementModelType[],
    edgelessBackground?: {
      zoom: number;
    }
    ...
    for (const block of blocks) {
      if (matchFlavours(block, ['workbench:image'])) {
      ...
      if (matchFlavours(block, ['workbench:note'])) {
      ...
      if (matchFlavours(block, ['workbench:frame'])) {
