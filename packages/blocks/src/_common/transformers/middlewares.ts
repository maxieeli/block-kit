...
import type { DeltaOperation, JobMiddleware } from '@maxiee/block_store';
...
export const replaceIdMiddleware: JobMiddleware = ({ slots, collection }) => {
  ...
  slots.afterImport.on(payload => {
    if (
      payload.type === 'block' &&
      payload.snapshot.flavour === 'workbench:database'
    ) {
    ...
    if (
      payload.type === 'block' &&
      ['workbench:paragraph', 'workbench:list'].includes(payload.snapshot.flavour)
    ) {
    ...
    if (
      payload.type === 'block' &&
      payload.snapshot.flavour === 'workbench:surface-ref'
    ) {
  ...
  slots.beforeImport.on(payload => {
    ...
    if (payload.type === 'block') {
      ...
      if (snapshot.flavour === 'workbench:page') {
        const index = snapshot.children.findIndex(
          c => c.flavour === 'workbench:surface'
        );
        if (index !== -1) {
      ...
      if (snapshot.flavour === 'workbench:surface') {
