...
const DEFAULT_ATTACHMENT_NAME = 'workbench-attachment';
...
export function addImageBlocks(
  ...
  const blockIds = imageFiles.map(file =>
    doc.addBlock('workbench:image', { size: file.size }, parent, parentIndex)
  );
  blockIds.map(
    (blockId, index) =>
      void uploadBlobForImage(editorHost, blockId, imageFiles[index])
...
export async function turnImageIntoCardView(block: ImageBlockComponent) {
  const doc = block.doc;
  if (!doc.schema.flavourSchemaMap.has('workbench:attachment')) {
  ...
  transformModel(model, 'workbench:attachment', attachmentProp);
}
