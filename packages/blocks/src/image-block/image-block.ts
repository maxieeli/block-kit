...
import type { WorkbenchImageCard } from './components/image-card.js';
...
> {
  ...
  @query('workbench-image-block-card')
  private accessor _imageCard: WorkbenchImageCard | null = null;

  @query('workbench-page-image')
  private accessor _pageImage: ImageBlockPageComponent | null = null;

  @query('workbench-edgeless-image')
  ...
  override renderBlock() {
    ...
    return html`
      <div
        class="workbench-image-container"
        style=${containerStyleMap}
        @click=${this._handleClick}
      >
        ${this.loading || this.error
          ? html`<workbench-image-block-card
              .block=${this}
            ></workbench-image-block-card>`
          : this.isInSurface
            ? html`<workbench-edgeless-image
                .url=${this.blobUrl}
                @error=${(_: CustomEvent<Error>) => {
                  this.error = true;
                }}
              ></workbench-edgeless-image>`
            : html`<workbench-page-image .block=${this}></workbench-page-image>`}
      </div>
...
declare global {
  interface HTMLElementTagNameMap {
    'workbench-image': ImageBlockComponent;
  }
}
