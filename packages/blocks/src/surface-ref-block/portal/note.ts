...
import type { Block } from '@maxiee/block_store';
import {
  type BlockModel,
  type BlockSelector,
  BlockViewType,
...
export class SurfaceRefNotePortal extends WithDisposable(ShadowlessElement) {
  ...
  override render() {
    ...
    const style = {
      ...
      border: `${EDGELESS_BLOCK_CHILD_BORDER_WIDTH}px none var(--workbench-black-10)`,
      background: `var(${background ?? DEFAULT_NOTE_BACKGROUND_COLOR})`,
      boxShadow: 'var(--workbench-note-shadow-sticker)',
      position: 'absolute',
      borderRadius: '0px',
