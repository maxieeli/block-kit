...
const portalMap = {
  'workbench:note': 'surface-ref-note-portal',
} as Record<BlockKit.EdgelessModelKeyType, string>;

const getPortalTag = (model: BlockModel) => {
  const tag = portalMap[model.flavour as BlockKit.EdgelessModelKeyType];
