...
import { assertExists, type Disposable, noop } from '@maxiee/block_global/utils';
import {
  css,
  html,
  nothing,
...
type RefElementModel = BlockKit.SurfaceElementModelType | FrameBlockModel;

@customElement('workbench-surface-ref')
@Peekable()
...
> {
  ...
  private get _shouldRender() {
    return (
      this.isConnected &&
      this.parentElement &&
      !this.parentBlockElement.closest('workbench-surface-ref')
    );
  }
  ...
  static override styles = css`
    ...
    .placeholder-reason {
      ...
      color: var(--workbench-warning-color);
      background-color: var(--workbench-background-error-color);
    }
    ...
    .ref-content {
      ...
      background: radial-gradient(
        var(--workbench-edgeless-grid-color) 1px,
        var(--workbench-background-primary-color) 1px
      );
    }
    ...
    .ref-viewport.frame {
      border-radius: 2px;
      border: 1px solid var(--workbench-black-30);
    }
    ...
    .ref-label .title > svg {
      color: var(--workbench-icon-secondary);
      display: inline-block;
  ...
  @query('workbench-surface-ref > block-caption-editor')
  ...
  private _renderRefContent(
    ...
    const edgelessBlocks =
      flavourOrType === 'workbench:frame' || flavourOrType === 'group'
    ...
    >
      <div
        class="ref-viewport ${flavourOrType === 'workbench:frame' ? 'frame' : ''}"
        style=${styleMap({
          width: `${w}px`,
          aspectRatio: `${w} / ${h}`,
        })}
  ...
  override connectedCallback() {
    ...
    this._isInSurface = parent?.flavour === 'workbench:surface';
  ...
  viewInEdgeless() {
    ...
    const pageService = this.std.spec.getService('workbench:page');
  ...
  override render() {
    ...
    return html`
      <div
        class="workbench-surface-ref"
        @click=${this._focusBlock}
        style=${styleMap({
          outline: this._focused
            ? '2px solid var(--workbench-primary-color)'
            : undefined,
        })}
      >
        ${content}
...
declare global {
  interface HTMLElementTagNameMap {
    'workbench-surface-ref': SurfaceRefBlockComponent;
  }
}
