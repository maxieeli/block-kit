...
import { assertEquals, assertExists, Slot } from '@maxiee/block_global/utils';
...
import type { WorkbenchPieMenuWidget } from './index.js';
...
@customElement('workbench-pie-menu')
export class PieMenu extends WithDisposable(LitElement) {
  ...
  @property({ attribute: false })
  accessor widgetElement!: WorkbenchPieMenuWidget;
  ...
  private selectChildWithIndex = (index: number) => {
    ...
    const node = activeNode.querySelector(
      `& > workbench-pie-node[index='${index}']`
    );
