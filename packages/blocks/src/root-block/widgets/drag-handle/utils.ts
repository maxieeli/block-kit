import type { BlockElement, EditorHost } from '@maxiee/block-std';
import {
  type BaseSelection,
  PathFinder,
...
export const insideDatabaseTable = (element: Element) => {
  return !!element.closest('.workbench-database-block-table');
};
...
export const getNoteId = (blockElement: BlockElement) => {
  ...
  while (element && element.flavour !== 'workbench:note') {
...
export const getClosestNoteBlock = (
  ...
  return isInsidePageEditor(editorHost)
    ? findClosestBlockElement(rootElement, point, 'workbench-note')
    : getHoveringNote(point)
        ?.closest('edgeless-block-portal-note')
        ?.querySelector('workbench-note');
};
...
export const getClosestBlockByPoint = (
  ...
  if (!closestNoteBlock || closestNoteBlock.closest('.workbench-surface-ref')) {
    return null;
  }
  ...
  const blockSelector =
    '.workbench-note-block-container > .workbench-block-children-container > [data-block-id]';
...
export function calcDropTarget(
  ...
  if (type === 'before') {
    ...
    if (
      allowSublist &&
      matchFlavours(model, ['workbench:list']) &&
      !hasChild &&
...
export function getDragHandleLeftPadding(blockElements: BlockElement[]) {
  const hasToggleList = blockElements.some(
    blockElement =>
      matchFlavours(blockElement.model, ['workbench:list']) &&
