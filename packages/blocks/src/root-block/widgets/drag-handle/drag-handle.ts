...
export const WORKBENCH_DRAG_HANDLE_WIDGET = 'workbench-drag-handle-widget';

@customElement(WORKBENCH_DRAG_HANDLE_WIDGET)
export class WorkbenchDragHandleWidget extends WidgetElement<
  RootBlockModel,
  EdgelessRootBlockComponent | PageRootBlockComponent
> {
  get optionRunner() {
    return WorkbenchDragHandleWidget.staticOptionRunner;
  }
  ...
  @query('.workbench-drag-handle-container')
  private accessor _dragHandleContainer!: HTMLDivElement;

  @query('.workbench-drag-handle-grabber')
  ...
  private _throttledPointerMoveHandler = throttle<UIEventHandler>(ctx => {
    ...
    if (element.closest('.workbench-drag-handle-container')) return;
  ...
  private _getDropResult = (state: PointerEventState): DropResult | null => {
    ...
    const isDraggedElementNote =
      ...
      matchFlavours(this.draggingElements[0].model, ['workbench:note']);
    ...
    if (isDraggedElementNote) {
      ...
      if (!matchFlavours(parentElement.model, ['workbench:note'])) return null;
    }
  ...
  private _changeCursorToGrabbing = () => {
    document.documentElement.classList.add('workbench-drag-preview-grabbing');
  };

  private _resetCursor = () => {
    document.documentElement.classList.remove('workbench-drag-preview-grabbing');
  };
  ...
  private _clickHandler: UIEventHandler = ctx => {
    ...
    const { target } = state.raw;
    const element = captureEventTarget(target);
    const insideDragHandle = !!element?.closest(WORKBENCH_DRAG_HANDLE_WIDGET);
  ...
  private _onDragEnd = (state: PointerEventState) => {
    ...
    if (!targetBlockId) {
      ...
      const isSurfaceComponent = selectedBlocks.some(block => {
        ...
        return matchFlavours(parent, ['workbench:surface']);
      });
  ...
  private _pointerOutHandler: UIEventHandler = ctx => {
    ...
    const inPage = !!relatedElement?.closest('.workbench-page-viewport');

    const inDragHandle = !!relatedElement?.closest(WORKBENCH_DRAG_HANDLE_WIDGET);
  ...
  override render() {
    ...
    return html`
      <div class="workbench-drag-handle-widget">
        <div class="workbench-drag-handle-container">
          <div class="workbench-drag-handle-grabber"></div>
        </div>
        <div class="workbench-drag-hover-rect" style=${hoverRectStyle}></div>
      </div>
    `;
  }

  static registerOption(option: DragHandleOption) {
    return WorkbenchDragHandleWidget.staticOptionRunner.register(option);
  }
}

declare global {
  interface HTMLElementTagNameMap {
    [WORKBENCH_DRAG_HANDLE_WIDGET]: WorkbenchDragHandleWidget;
  }
}
