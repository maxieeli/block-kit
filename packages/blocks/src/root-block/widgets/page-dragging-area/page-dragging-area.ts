...
import { assertExists, assertInstanceOf } from '@maxiee/block_global/utils';
...
export const WORKBENCH_PAGE_DRAGGING_AREA_WIDGET =
  'workbench-page-dragging-area-widget';

@customElement(WORKBENCH_PAGE_DRAGGING_AREA_WIDGET)
export class WorkbenchPageDraggingAreaWidget extends WidgetElement<
  RootBlockModel,
...
> {
    ...
    const getAllNodeFromTree = (): BlockElement[] => {
      ...
      this.host.view.walkThrough(node => {
        ...
        if (
          ...
          !WorkbenchPageDraggingAreaWidget.excludeFlavours.includes(
            view.model.flavour
          )
        ) {
  ...
  static excludeFlavours: string[] = ['workbench:note', 'workbench:surface'];
  ...
  override render() {
    ...
    return html`
      ...
      <div class="workbench-page-dragging-area" style=${styleMap(style)}></div>
    `;
  }
}
...
function isDragArea(e: PointerEventState) {
  ...
  return block && matchFlavours(block.model, ['workbench:page', 'workbench:note']);
}

declare global {
  interface HTMLElementTagNameMap {
    [WORKBENCH_PAGE_DRAGGING_AREA_WIDGET]: WorkbenchPageDraggingAreaWidget;
  }
}
