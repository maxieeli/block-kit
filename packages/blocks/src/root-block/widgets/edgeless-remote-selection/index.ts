...
import type { UserInfo } from '@maxiee/block_store';
...
export const WORKBENCH_EDGELESS_REMOTE_SELECTION_WIDGET =
  'workbench-edgeless-remote-selection-widget';

@customElement(WORKBENCH_EDGELESS_REMOTE_SELECTION_WIDGET)
...
> {
  ...
  static override styles = css`
    ...
    .remote-username {
      ...
      max-width: 160px;
      padding: 0px 3px;
      border: 1px solid var(--workbench-pure-black-20);
    ...
    remoteSelectionsMap.forEach((selections, clientId) => {
      selections.forEach(selection => {
        ...
        const elements = selection.elements
          ...
          .filter(element => element) as BlockKit.EdgelessModelType[];
  ...
  override render() {
    ...
    return html`
      <div class="workbench-edgeless-remote-selection">${rects}${cursors}</div>
    `;
  }
}
...
declare global {
  interface HTMLElementTagNameMap {
    [WORKBENCH_EDGELESS_REMOTE_SELECTION_WIDGET]: EdgelessRemoteSelectionWidget;
  }
}
