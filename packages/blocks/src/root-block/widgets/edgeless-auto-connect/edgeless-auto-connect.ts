...
export const WORKBENCH_EDGELESS_AUTO_CONNECT_WIDGET =
  'workbench-edgeless-auto-connect-widget';

@customElement(WORKBENCH_EDGELESS_AUTO_CONNECT_WIDGET)
...
> {
  ...
  private _initLabels() {
    ...
    const surfaceRefs = service.doc
      .getBlocksByFlavour('workbench:surface-ref')
    ...
    const updateLabels = requestThrottledConnectFrame(() => {
      ...
      notes.forEach(note => {
        ...
        note.children.forEach(model => {
          if (matchFlavours(model, ['workbench:surface-ref'])) {
      ...
      (element, i) => {
        ...
        for (let j = 0; j < count; j++) {
          ...
          components.push(html`
            ...
            >
              ${index}
              <workbench-tooltip tip-position="bottom">
    ...
    return html`${repeat(
      ...
      note => {
        ...
        const style = styleMap({
          ...
          borderRadius: '50%',
          backgroundColor: 'var(--workbench-text-secondary-color)',
...
declare global {
  interface HTMLElementTagNameMap {
    'workbench-edgeless-auto-connect-widget': EdgelessAutoConnectWidget;
  }
}
