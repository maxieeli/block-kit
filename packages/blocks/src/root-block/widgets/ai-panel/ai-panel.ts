...
import type { BaseSelection } from '@maxiee/block-std';
...
import { WORKBENCH_FORMAT_BAR_WIDGET } from '../format-bar/format-bar.js';
import {
  WORKBENCH_VIEWPORT_OVERLAY_WIDGET,
  type WorkbenchViewportOverlayWidget,
} from '../viewport-overlay/viewport-overlay.js';
import type { AIPanelGenerating } from './components/index.js';
import type { WorkbenchAIPanelState, WorkbenchAIPanelWidgetConfig } from './type.js';

export const WORKBENCH_AI_PANEL_WIDGET = 'workbench-ai-panel-widget';

@customElement(WORKBENCH_AI_PANEL_WIDGET)
export class WorkbenchAIPanelWidget extends WidgetElement {
  get viewportOverlayWidget() {
    ...
    return rootId
      ? (this.host.view.getWidget(
          WORKBENCH_VIEWPORT_OVERLAY_WIDGET,
          rootId
        ) as WorkbenchViewportOverlayWidget)
      : null;
  }
  ...
  accessor config: WorkbenchAIPanelWidgetConfig | null = null;

  @property()
  accessor state: WorkbenchAIPanelState = 'hidden';
  ...
  private _calcPositionOptions(
    ...
    {
      const rootService = this.host.std.spec.getService('workbench:page');
  ...
  protected override willUpdate(changed: PropertyValues): void {
    ...
    if (prevState) {
      ...
      const formatBar = rootBlockId
        ? this.host.view.getWidget(WORKBENCH_FORMAT_BAR_WIDGET, rootBlockId)
        : null;
  ...
  showDiscardModal = () => {
    const notification =
      this.host.std.spec.getService('workbench:page').notificationService;
    if (!notification) {
