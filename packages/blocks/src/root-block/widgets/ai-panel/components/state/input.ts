...
export class AIPanelInput extends WithDisposable(LitElement) {
  static override styles = css`
    ...
    .arrow {
      ...
      gap: 10px;
      border-radius: 4px;
      background: var(--workbench-black-10, rgba(0, 0, 0, 0.1));

      svg {
        width: 16px;
        height: 16px;
        color: var(--workbench-pure-white, #fff);
      }
    }
    .arrow[data-active] {
      background: var(--workbench-brand-color, #1e96eb);
    }
    ...
    return html`<div class="root">
      ...
      <div class="textarea-container">
        ...
        >
          ${ArrowUpBigIcon}
          ${this._hasContent
            ? html`<workbench-tooltip .offset=${12}>Send to AI</workbench-tooltip>`
            : nothing}
        </div>
      </div>
    </div>`;
