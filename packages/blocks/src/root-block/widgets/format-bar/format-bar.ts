...
import { assertExists, DisposableGroup } from '@maxiee/block_global/utils';
import {
  autoUpdate,
  computePosition,
  inline,
...
export const WORKBENCH_FORMAT_BAR_WIDGET = 'workbench-format-bar-widget';

@customElement(WORKBENCH_FORMAT_BAR_WIDGET)
export class WorkbenchFormatBarWidget extends WidgetElement {
  ...
  @query(`.${WORKBENCH_FORMAT_BAR_WIDGET}`)
  ...
  private _shouldDisplay() {
    ...
    if (
      ...
      this._selectedBlockElements?.[0]?.flavour === 'workbench:surface-ref'
    ) {
      return false;
    }
    ...
    const aiPanel = rootBlockId
      ? this.host.view.getWidget('workbench-ai-panel-widget', rootBlockId)
      : null;
  ...
  override connectedCallback() {
    ...
    if (!Object.hasOwn(widgets, WORKBENCH_FORMAT_BAR_WIDGET)) {
      return;
    }
  ...
  override render() {
    ...
    return html`<div
      class="${WORKBENCH_FORMAT_BAR_WIDGET}"
...
declare global {
  interface HTMLElementTagNameMap {
    [WORKBENCH_FORMAT_BAR_WIDGET]: WorkbenchFormatBarWidget;
  }
}
