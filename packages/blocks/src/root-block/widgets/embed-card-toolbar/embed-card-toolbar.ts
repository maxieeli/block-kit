...
import { type BlockModel, DocCollection } from '@maxiee/block_store';
import {
  autoUpdate,
  computePosition,
  flip,
...
export const WORKBENCH_EMBED_CARD_TOOLBAR_WIDGET = 'workbench-embed-card-toolbar';

@customElement(WORKBENCH_EMBED_CARD_TOOLBAR_WIDGET)
...
> {
  ...
  private get _embedViewButtonDisabled() {
    return (
      isEmbedLinkedDocBlock(this.model) &&
      !!this.blockElement.closest('workbench-embed-synced-doc-block')
    );
  }
      ...
      >
          ...
          ? html`
              ...
              >
                ...
                <workbench-tooltip .offset=${12}
                  >${'Click to copy link'}</workbench-tooltip
                >
              </icon-button>
        ...
        ${isEmbedLinkedDocBlock(model) || isEmbedSyncedDocBlock(model)
          ? html`
              ...
              >
                ...
                ${OpenIcon}
                <workbench-tooltip .offset=${12}
                  >${'Open this doc'}</workbench-tooltip
                >
              </icon-button>
        ...
        <div class="embed-card-toolbar-button view-selector">
          ...
          ${this._canConvertToEmbedView || this._isEmbedView
            ? html`
                ...
                >
                  ${EmbedWebIcon}
                  <workbench-tooltip .offset=${12}>${'Embed view'}</workbench-tooltip>
                </icon-button>
              `
            : nothing}
        </div>
        ...
        ${this._canShowCardStylePanel(model)
          ? html`
              ...
              >
                ${PaletteIcon}
                <workbench-tooltip .offset=${12}>${'Card style'}</workbench-tooltip>
              </icon-button>
            `
          : nothing}
        ...
        >
          ${CaptionIcon}
          <workbench-tooltip .offset=${12}>${'Add Caption'}</workbench-tooltip>
        </icon-button>
...
declare global {
  interface HTMLElementTagNameMap {
    [WORKBENCH_EMBED_CARD_TOOLBAR_WIDGET]: EmbedCardToolbar;
  }
}
