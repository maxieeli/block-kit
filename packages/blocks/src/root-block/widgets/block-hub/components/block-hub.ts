...
@customElement('workbench-block-hub')
export class BlockHub extends WithDisposable(ShadowlessElement) {
  ...
  private _onDragStart = (event: DragEvent) => {
    ...
    const data: {
      ...
      flavour: blockHubElement.getAttribute('workbench-flavour'),
      type: blockHubElement.getAttribute('workbench-type') ?? undefined,
    };
    event.dataTransfer.setData('workbench/block-hub', JSON.stringify(data));
  ...
  private _onDrop = async (e: DragEvent) => {
    ...
    if (!e.dataTransfer.getData('workbench/block-hub')) return;
    ...
    const data = dataTransfer.getData('workbench/block-hub');
    const models = [];
    const props = JSON.parse(data);
    const isDatabase = props.flavour === 'workbench:database';

    // TO DO: fix image loading state for block hub
    if (
      props.flavour === 'workbench:image' &&
      props.type === 'image' &&
      lastModelState &&
      lastType !== 'none' &&
      lastType !== 'database'
      ...
      const imageService = this._editorHost.spec.getService('workbench:image');
    ...
    } else if (props.flavour === 'workbench:bookmark') {
    ...
    if (lastModelState && lastType !== 'none') {
      ...
      if (isDatabase) {
        const service =
          this._rootElement.std.spec?.getService('workbench:database');
    ...
    if (focusId && parentModel) {
      ...
      assertExists(model);
      if (isDatabase) {
        const service = rootElement.std.spec?.getService('workbench:database');
        service.initDatabaseBlock(
          doc,
          model,
          model.id,
  ...
  private _onClickCard = async (
    ...
    const workbenchType = blockHubElement.getAttribute('workbench-type');
    assertExists(workbenchType);
    const data: {
      flavour: string;
      type?: string;
    } = {
      flavour: blockHubElement.getAttribute('workbench-flavour') ?? '',
      type: workbenchType ?? undefined,
    };
    const doc = this._editorHost.doc;
    const models = [];

    const defaultNoteBlock =
      doc.root?.children.findLast(block => block.flavour === 'workbench:note') ??
      doc.addBlock('workbench:note', {}, doc.root?.id);

    // add to end
    let lastId;

    // TO DO: fix image loading state for blockhub
    if (data.flavour === 'workbench:image' && data.type === 'image') {
      const imageFiles = await getImageFilesFromLocal();
    ...
    } else if (data.flavour === 'workbench:bookmark') {
      await toggleEmbedCardCreateModal(
        this._editorHost,
        'Links',
        'The added link will be displayed as a card view.',
    ...
    models.forEach(model => {
      lastId = doc.addBlock(
        (model.flavour ?? 'workbench:paragraph') as never,
        model,
        defaultNoteBlock
      );
    });
  ...
  private _onClickOutside = (e: MouseEvent) => {
    ...
    if (target instanceof HTMLElement && !target.closest('workbench-block-hub')) {
      this._hideCardList();
    }
  };
      ...
      >
        ...
        <div class=${classes} role="menuitem" style="cursor:pointer;">
          ${this._expanded
            ...
            : [
                BlockHubIcon,
                html`<workbench-tooltip tip-position="left"
                  >Insert blocks</workbench-tooltip
                >`,
              ]}
        </div>
      </div>
