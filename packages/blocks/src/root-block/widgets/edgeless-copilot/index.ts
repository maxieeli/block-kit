...
import type { WorkbenchAIPanelWidget } from '../ai-panel/ai-panel.js';
import { WORKBENCH_AI_PANEL_WIDGET } from '../ai-panel/ai-panel.js';
import { EdgelessCopilotPanel } from '../edgeless-copilot-panel/index.js';

export const WORKBENCH_EDGELESS_COPILOT_WIDGET = 'workbench-edgeless-copilot-widget';

@customElement(WORKBENCH_EDGELESS_COPILOT_WIDGET)
...
> {
  ...
  static override styles = css`
    .copilot-selection-rect {
      ...
      border-radius: 4px;
      border: 2px dashed var(--workbench-brand-color, #1e96eb);
    }
  `;
      ...
      this._listenClickOutsideId = requestConnectedFrame(() => {
        ...
        const off = this.blockElement.dispatcher.add('pointerDown', ctx => {
          ...
          const aiPanel = this.host.view.getWidget(
            WORKBENCH_AI_PANEL_WIDGET,
            this.doc.root!.id
          ) as WorkbenchAIPanelWidget;
  ...
  override render() {
    ...
    return html`<div class="workbench-edgeless-ai">
      <div
...
declare global {
  interface HTMLElementTagNameMap {
    [WORKBENCH_EDGELESS_COPILOT_WIDGET]: EdgelessCopilotWidget;
  }
}
