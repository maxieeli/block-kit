...
import { assertType } from '@maxiee/block_global/utils';
...
export function insertContent(
  ...
  model: BlockModel,
  text: string,
  attributes?: WorkbenchTextAttributes
) {
  if (!model.text) {
...
export function insideDatabase(model: BlockModel) {
  return isInsideBlockByFlavour(model.doc, model, 'workbench:database');
}

export function insideDataView(model: BlockModel) {
  return isInsideBlockByFlavour(model.doc, model, 'workbench:data-view');
}

export function insideEdgelessText(model: BlockModel) {
  return isInsideBlockByFlavour(model.doc, model, 'workbench:edgeless-text');
}

export function createDatabaseBlockInNextLine(model: BlockModel) {
  let parent = model.doc.getParent(model);
  while (parent && parent.flavour !== 'workbench:note') {
    model = parent;
  ...
  return model.doc.addBlock('workbench:database', {}, parent, index + 1);
}
