...
import { Slice, Text } from '@maxiee/block_store';
...
import type { WorkbenchLinkedDocWidget } from '../linked-doc/index.js';
...
export type SlashMenuConfig = {
  triggerKeys: string[];
  ignoreBlockTypes: BlockKit.Flavour[];
  items: SlashMenuItem[];
  maxHeight: number;
...
export const defaultSlashMenuConfig: SlashMenuConfig = {
  triggerKeys: ['/'],
  ignoreBlockTypes: ['workbench:code'],
  maxHeight: 344,
  tooltipTimeout: 800,
  items: [
    // ---------------------------------------------------------
    ...
    ...textConversionConfigs
      .filter(i => i.flavour === 'workbench:code')
    ...
    { groupName: 'List' },
    ...textConversionConfigs
      .filter(i => i.flavour === 'workbench:list')
      .map(createConversionItem),
    ...
    },
    {
      ...
      showWhen: ({ rootElement }) => {
        const linkedDocWidgetEle =
          rootElement.widgetElements['workbench-linked-doc-widget'];
      ...
      action: ({ model, rootElement }) => {
        ...
        const linkedDocWidget = widgetEle as WorkbenchLinkedDocWidget;
    ...
    { groupName: 'Content & Media' },
    {
      ...
      showWhen: ({ model }) =>
        model.doc.schema.flavourSchemaMap.has('workbench:image') &&
        !insideDatabase(model),
      action: async ({ rootElement, model }) => {
        ...
        const imageService = rootElement.host.spec.getService('workbench:image');
    ...
    },
    {
      ...
      showWhen: ({ model }) =>
        model.doc.schema.flavourSchemaMap.has('workbench:bookmark') &&
        !insideDatabase(model),
      action: async ({ rootElement, model }) => {
    ...
    },
    {
      ...
      alias: ['file'],
      showWhen: ({ model }) =>
        model.doc.schema.flavourSchemaMap.has('workbench:attachment') &&
        !insideDatabase(model),
      action: async ({ rootElement, model }) => {
        ...
        const attachmentService =
          rootElement.host.spec.getService('workbench:attachment');
    ...
    },
    {
      ...
      showWhen: ({ model }) =>
        model.doc.schema.flavourSchemaMap.has('workbench:embed-youtube') &&
        !insideDatabase(model),
      action: async ({ rootElement, model }) => {
        const parentModel = rootElement.doc.getParent(model);
        if (!parentModel) {
    ...
    },
    {
      ...
      showWhen: ({ model }) =>
        model.doc.schema.flavourSchemaMap.has('workbench:embed-github') &&
        !insideDatabase(model),
      action: async ({ rootElement, model }) => {
        const parentModel = rootElement.doc.getParent(model);
        if (!parentModel) {
...

    {
      ...
      showWhen: ({ model }) =>
        model.doc.schema.flavourSchemaMap.has('workbench:embed-figma') &&
        !insideDatabase(model),
      action: async ({ rootElement, model }) => {
        const parentModel = rootElement.doc.getParent(model);
        if (!parentModel) {
...

    {
      ...
      icon: LoomIcon,
      showWhen: ({ model }) =>
        model.doc.schema.flavourSchemaMap.has('workbench:embed-loom') &&
        !insideDatabase(model),
      action: async ({ rootElement, model }) => {
        const parentModel = rootElement.doc.getParent(model);
        if (!parentModel) {
    ...
    ({ model, rootElement }) => {
      ...
      const frameModels = doc
        .getBlocksByFlavour('workbench:frame')
    ...
    { groupName: 'Database' },
    {
      ...
      showWhen: ({ model }) =>
        model.doc.schema.flavourSchemaMap.has('workbench:database') &&
        !insideDatabase(model) &&
        !insideEdgelessText(model),
      action: ({ rootElement, model }) => {
