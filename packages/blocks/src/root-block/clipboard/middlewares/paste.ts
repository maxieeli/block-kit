...
class PasteTr {
  ...
  constructor(
    ...
    this.firstSnapshotIsPlainText =
      this.firstSnapshot.flavour === 'workbench:paragraph' &&
  ...
  focusPasted = () => {
    ...
    host.updateComplete
      .then(() => {
        ...
        assertExists(target);
        if (!lastModel.text) {
          if (matchFlavours(lastModel, ['workbench:image'])) {
  ...
  merge() {
    if (this.fromPointState.model.flavour === 'workbench:code' && !this.to) {
      this._mergeCode();
      return;
    }
  ...
  convertToLinkedDoc = async (std: EditorHost['std']) => {
    const quickSearchService =
      std.spec.getService('workbench:page').quickSearchService;
...
function flatNote(snapshot: SliceSnapshot) {
  if (snapshot.content[0]?.flavour === 'workbench:note') {
