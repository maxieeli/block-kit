...
import type { SurfaceSelection } from '@maxiee/block-std';
import { BlockElement } from '@maxiee/block-std';
import { IS_WINDOWS } from '@maxiee/block_global/env';
import { assertExists, throttle } from '@maxiee/block_global/utils';
...
@customElement('workbench-edgeless-root')
...
> {
  ...
  get surfaceBlockModel() {
    return this.model.children.find(
      child => child.flavour === 'workbench:surface'
    ) as SurfaceBlockModel;
  }

  static override styles = css`
    workbench-edgeless-root {
  ...
  /**
   * Adds a new note with the given point on the workbench-editor-container.
   *
   * @param: point Point
  ...
  async addImages(
    ...
    const imageService = this.host.spec.getService('workbench:image');
  ...
  async addAttachments(files: File[], point?: IVec2): Promise<string[]> {
    ...
    const attachmentService = this.host.spec.getService('workbench:attachment');
  ...
  setSelection(noteId: string, _active = true, blockId: string, point?: Point) {
    const noteBlock = this.service.blocks
      .filter(block => block.flavour === 'workbench:note')
      .find(b => b.id === noteId);
...
declare global {
  interface HTMLElementTagNameMap {
    'workbench-edgeless-root': EdgelessRootBlockComponent;
  }
}
