import { assertExists, assertType } from '@maxiee/block_global/utils';
import type { BlockSnapshot, SnapshotReturn } from '@maxiee/block_store';
...
export const replaceIdMiddleware = (job: TemplateJob) => {
  ...
  const regenerateBlockId = (data: SlotBlockPayload['data']) => {
    ...
    if (blockJson.flavour === 'workbench:surface-ref') {
      assertType<
        SnapshotReturn<{
          reference: string;
        }>
    ...
    if (blockJson.flavour === 'workbench:surface') {
...
export const createStickerMiddleware = (
  ...
  return (job: TemplateJob) => {
    ...
    const changeInserPosition = (blockJson: BlockSnapshot) => {
      if (blockJson.flavour === 'workbench:image' && blockJson.props.xywh) {
        const bound = Bound.deserialize(blockJson.props['xywh'] as string);
    ...
    const generateIndexMap = () => {
      ...
      job.walk(block => {
        if (block.props.index) {
          if (block.flavour === 'workbench:frame') {
            frameList.push({
              id: block.id,
        ...
        if (block.flavour === 'workbench:surface') {
          Object.entries(
      ...
      frameList.forEach(index => {
        indexMap.set(index.id, generateIndex('workbench:frame'));
      });
