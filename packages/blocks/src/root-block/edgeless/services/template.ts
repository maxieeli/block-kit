import { assertExists, assertType, Slot } from '@maxiee/block_global/utils';
import type { BlockModel, Y } from '@maxiee/block_store';
import {
  type BlockSnapshot,
  type DocSnapshot,
  DocSnapshotSchema,
 ...
 */
const DEFERED_BLOCK = ['workbench:surface', 'workbench:surface-ref'] as const;

/**
 * Those block should not be inserted directly
 * it should be merged with current existing block
 */
const MERGE_BLOCK = ['workbench:surface', 'workbench:page'] as const;
...
export class TemplateJob {
  ...
  private _mergeSurfaceElements(
    ...
    const schema =
      this.model.doc.collection.schema.flavourSchemaMap.get('workbench:surface');
