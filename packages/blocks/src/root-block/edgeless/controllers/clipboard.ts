...
const BLOCKKIT_SURFACE = 'blockkit/surface';
...
export class EdgelessClipboardController extends PageClipboard {
  ...
  private _onCopy = (
    ...
    this.std.clipboard
      .writeToClipboard(async _items => {
        ...
        return {
          ..._items,
          [BLOCKKIT_SURFACE]: JSON.stringify(data),
        };
      })
      .catch(console.error);
  };
  ...
  private _onPaste = async (_context: UIEventStateContext) => {
    ...
    if (isUrlInClipboard(data)) {
      ...
      const flavour = pageId
        ...
        : embedOptions
          ? (embedOptions.flavour as BlockKit.EdgelessModelKeyType)
          : 'workbench:bookmark';
      const style = pageId
        ? 'vertical'
        : embedOptions
    ...
    try {
      ...
      const mayBeSurfaceDataJson = json[BLOCKKIT_SURFACE];
  ...
  private _updatePastedElementsIndex(
    ...
    function compare(
      a: BlockKit.EdgelessModelType,
      b: BlockKit.EdgelessModelType
    ) {
  ...
  private async _edgelessToCanvas(
    ...
    bound: IBound,
    nodes?: BlockKit.EdgelessBlockModelType[],
    canvasElements: BlockKit.SurfaceModelType[] = [],
    {
      background,
    ...
    const container = rootElement.querySelector(
      '.workbench-block-children-container'
    );
    if (!container) return;
    ...
    const html2canvasOption = {
      ...
      onclone: async function (documentClone: Document, element: HTMLElement) {
        ...
        const layer = documentClone.querySelector('.workbench-edgeless-layer');
    ...
    const _drawTopLevelBlock = async (
      block: BlockKit.EdgelessBlockModelType,
      isInFrame = false
    ) => {
    ...
    const nodeElements =
      ...
      (edgeless.service.pickElementsByBound(
        bound,
        'blocks'
      ) as BlockKit.EdgelessBlockModelType[]);
    for (const nodeElement of nodeElements) {
      await _drawTopLevelBlock(nodeElement);

      if (matchFlavours(nodeElement, ['workbench:frame'])) {
        const blocksInsideFrame: BlockKit.EdgelessBlockModelType[] = [];
        this.edgeless.service.frame
          .getElementsInFrame(nodeElement, false)
          .forEach(ele => {
            if (isTopLevelBlock(ele)) {
              blocksInsideFrame.push(ele as BlockKit.EdgelessBlockModelType);
            } else {
              canvasElements.push(ele as BlockKit.SurfaceModelType);
            }
          });
  ...
  async toCanvas(
    blocks: BlockKit.EdgelessBlockModelType[],
    shapes: BlockKit.SurfaceModelType[],
...
export async function prepareClipboardData(
  selectedAll: BlockKit.EdgelessModelType[],
  std: BlockStdScope
) {
  const job = new Job({
