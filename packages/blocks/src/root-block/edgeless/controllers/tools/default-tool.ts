...
import { DisposableGroup, noop } from '@maxiee/block_global/utils';
...
export class DefaultToolController extends EdgelessToolController<DefaultTool> {
  ...
  private _toBeMoved: BlockKit.EdgelessModelType[] = [];
  ...
  private _isDraggable(element: BlockKit.EdgelessModelType) {
    return !(
  ...
  onContainerDblClick(e: PointerEventState) {
    ...
    if (
      ...
      e.raw.target.classList.contains('workbench-note-mask')
    ) {
