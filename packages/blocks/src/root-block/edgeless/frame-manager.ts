...
export class EdgelessFrameManager {
  ...
  selectFrame(eles: BlockKit.EdgelessModelType[]) {
  ...
  getElementsInFrame(frame: FrameBlockModel, fullyContained = true) {
    const bound = Bound.deserialize(frame.xywh);
    const elements: BlockKit.EdgelessModelType[] =
  ...
  createFrameOnSelected() {
    const frames = this._rootService.frames;
    const surfaceModel =
      this._rootService.doc.getBlockByFlavour('workbench:surface')[0];
...
export function getNotesInFrame(
  ...
  return (doc.getBlockByFlavour('workbench:note') as NoteBlockModel[]).filter(
    ele => {
