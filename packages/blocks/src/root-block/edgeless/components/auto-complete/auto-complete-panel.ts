...
export class EdgelessAutoCompletePanel extends WithDisposable(LitElement) {
  static override styles = css`
    ...
    .row-button {
      ...
      font-style: normal;
      font-weight: 500;
      border: 1px solid var(--workbench-border-color, #e3e2e4);
  ...
  private _showNoteOverlay() {
    ...
    if (isShape(this.currentSource)) {
      ...
      color = `--workbench-note-background-${tag}`;
    } else {
  ...
  private _showFrameOverlay() {
    ...
    const computedStyle = getComputedStyle(this.edgeless);
    const strokeColor = computedStyle.getPropertyValue('--workbench-black-30');
  ...
  private _addNote() {
    ...
    doc.addBlock('workbench:paragraph', { type: 'text' }, id);
