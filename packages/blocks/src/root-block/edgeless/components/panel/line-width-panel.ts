...
export class EdgelessLineWidthPanel extends WithDisposable(LitElement) {
  static override styles = css`
    ...
    .drag-handle {
      ...
      border-radius: 50%;
      background-color: var(--workbench-icon-color);
      z-index: 3;
    }
  ...
  private _updateIconsColor = () => {
    ...
    leftIcons.forEach(
      icon => (icon.style.backgroundColor = 'var(--workbench-icon-color)')
    );
    rightIcons.forEach(
      icon => (icon.style.backgroundColor = 'var(--workbench-border-color)')
    );
  };
  ...
  override render() {
    return html`<style>
      ...
      >
        ...
        ${this.hasTooltip
          ? html`<workbench-tooltip .offset=${8}>Thickness</workbench-tooltip>`
          : nothing}
      </div>`;
  }
}
