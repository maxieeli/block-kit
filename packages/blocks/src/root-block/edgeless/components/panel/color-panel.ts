...
export const DEFAULT_BRUSH_COLOR = '--workbench-palette-line-blue';
...
function BorderedHollowCircle(color: CssVariableName) {
  ...
  const style = {
    fill: `var(${color})`,
    stroke: 'var(--workbench-border-color)',
  };
  return html`
    <svg
      width="16"
...
export function ColorUnit(
  ...
  const borderStyle =
    isSameColorWithBackground(color) && !hollowCircle
      ? {
          border: '0.5px solid var(--workbench-border-color)',
        }
      : {};
  ...
  override render() {
    ...
    if (!hollowCircle) {
      ...
      if (isSameColorWithBackground(color)) {
        style.border = '0.5px solid var(--workbench-border-color)';
      }
    }
    return html`<div
      class="color-unit"
...
export const colorContainerStyles = css`
  ...
  .color-container[active]:after {
    ...
    width: 20px;
    height: 20px;
    border: 0.5px solid var(--workbench-primary-color);
    border-radius: 50%;
    box-sizing: border-box;
    content: attr(data-letter);
  }
