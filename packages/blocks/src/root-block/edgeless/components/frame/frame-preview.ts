...
import { debounce, DisposableGroup } from '@maxiee/block_global/utils';
import type { BlockModel } from '@maxiee/block_store';
import { type Block, BlockViewType, type Doc, nanoid } from '@maxiee/block_store';
...
type RefElement = Exclude<BlockKit.EdgelessModelType, NoteBlockModel>;
...
export class FramePreview extends WithDisposable(ShadowlessElement) {
  ...
  private get _surfaceService() {
    return this.host?.std.spec.getService('workbench:surface');
  }

  private get _surfaceRefService() {
    return this.host.spec.getService('workbench:surface-ref');
  }
      ...
      <div
        style=${styleMap({
          backgroundColor: referencedModel.background
            ...
            : 'var(--workbench-platte-transparent)',
          borderRadius: '4px',
        })}
      >
        <div
