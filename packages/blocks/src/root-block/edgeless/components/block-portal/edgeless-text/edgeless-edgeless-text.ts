...
export class EdgelessBlockPortalEdgelessText extends EdgelessPortalBase<EdgelessTextBlockModel> {
  ...
  override firstUpdated(props: PropertyValueMap<unknown>) {
    ...
    this.disposables.addFromEvent(this._textContainer, 'click', e => {
      ...
      if (isTop) {
        ...
        if (
          !firstChild ||
          !matchFlavours(firstChild, ['workbench:list', 'workbench:paragraph'])
        ) {
        ...
        if (
          !lastChild ||
          !matchFlavours(lastChild, ['workbench:list', 'workbench:paragraph'])
        ) {
          newParagraphId = this.edgeless.doc.addBlock(
            'workbench:paragraph',
            {},
  ...
  override render() {
    ...
    const containerStyle: StyleInfo = {
      ...
      padding: '10px',
      border: `1px solid ${this._editing ? 'var(--workbench—primary—color, #1e96eb)' : 'transparent'}`,
      borderRadius: '4px',
