...
export class EdgelessNoteMask extends WithDisposable(ShadowlessElement) {
  ...
  protected override firstUpdated() {
    const maskDOM = this.renderRoot!.querySelector('.workbench-note-mask');
...
export class EdgelessBlockPortalNote extends EdgelessPortalBase<NoteBlockModel> {
  static override styles = css`
    ...
    .edgeless-block-portal-note:has(.workbench-embed-synced-doc-container.editing)
      > .note-background {
  ...
  @query('workbench-note')
  private accessor _workbenchNote!: HTMLDivElement;
  ...
  private _handleClickAtBackground(e: MouseEvent) {
    e.stopPropagation();
    if (!this._workbenchNote || !this._editing) return;

    const rect = this._workbenchNote.getBoundingClientRect();
  ...
  private _tryAddParagraph(x: number, y: number) {
    ...
    if (
      (!nearestModel.text ||
        !matchFlavours(nearestModel, ['workbench:paragraph', 'workbench:list'])) &&
      (!siblingModel ||
        !siblingModel.text ||
        !matchFlavours(siblingModel, ['workbench:paragraph', 'workbench:list']))
    ) {
  ...
  private _collapsedContent() {
    if (!this._isShowCollapsedContent || !this._workbenchNote) {
      return nothing;
    }
    ...
    return html`
      <div
        style=${styleMap({
          ...
          opacity: 0.5,
          pointerEvents: 'none',
          borderLeft: '2px var(--workbench-blue) solid',
          borderBottom: '2px var(--workbench-blue) solid',
          borderRight: '2px var(--workbench-blue) solid',
  ...
  override firstUpdated() {
    ...
    const observer = new MutationObserver(() => {
      const workbenchNote = this._workbenchNote;
      if (!this._workbenchNote) return;
      const rect = workbenchNote.getBoundingClientRect();
      const zoom = this.edgeless.service.viewport.zoom;
      const scale = this.model.edgeless.scale ?? 1;
      this._noteFullHeight =
        rect.height / scale / zoom + 2 * EDGELESS_BLOCK_CHILD_PADDING;
  ...
  override render() {
    ...
    const backgroundStyle = {
      ...
      } var(--workbench-black-10)`,
      boxShadow: this._editing
        ? 'var(--workbench-active-shadow)'
        : !shadowType
          ? 'none'
          : `var(${shadowType})`,
    };
