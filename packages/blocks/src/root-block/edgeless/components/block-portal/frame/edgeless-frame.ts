        ...
        if (
          (payload.type === 'update' &&
            ...
            edgeless.doc.getBlock(payload.id)?.model instanceof
              FrameBlockModel) ||
          (payload.type === 'add' && payload.flavour === 'workbench:frame')
        ) {
          this._nestedFrame = this._isInsideFrame();
        }
      })
            ...
            <div
              style=${styleMap({
                ...
                background: nestedFrame
                  ? 'var(--workbench-white)'
                  : 'var(--workbench-text-primary-color)',
                color: nestedFrame
                  ? 'var(--workbench-text-secondary-color)'
                  : 'var(--workbench-white)',
                border: nestedFrame
                  ? '1px solid var(--workbench-border-color)'
                  : 'none',
              })}
              class="workbench-frame-title"
            >
              ${this._frameTitle}
            </div>
          `
