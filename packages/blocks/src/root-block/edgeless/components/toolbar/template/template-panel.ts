...
export class EdgelessTemplatePanel extends WithDisposable(LitElement) {
  ...
  static override styles = css`
    ...
    .search-bar {
      ...
      border-bottom: 1px solid var(--workbench-divider-color);
    ...
    .category-entry:hover {
      color: var(--workbench-text-primary-color);
      background-color: var(--workbench-background-tertiary-color);
    }
    ...
    .template-item > svg {
      ...
      width: 135px;
      height: 80px;
      color: var(--workbench-background-primary-color);
    }
    ...
    /* .template-item:hover::before {
      ...
      background-color: var(--workbench-primary-color);
    ...
    .template-item:hover::after {
      ...
      height: 100%;
      box-sizing: border-box;
      border: 1px solid var(--workbench-black-10);
      border-radius: 4px;
    ...
    .template-item.loading > workbench-template-loading {
      position: absolute;
      left: 50%;
      top: 50%;
      transform: translate(-50%, -50%);
    ...
    .arrow {
      ...
      color: var(--workbench-background-overlay-panel-color);
    }
  `;
    ...
    if (type === 'sticker') {
      middlewares.push(
        createStickerMiddleware(center, () =>
          service.layer.generateIndex('workbench:image')
        )
      );
    }
        ...
        <div class="template-viewport">
          <div class="template-scrollcontent" data-scrollable>
            <div class="template-list">
              ${this._loading
                ? html`<workbench-template-loading
                    style=${styleMap({
                      position: 'absolute',
                      left: '50%',
                      top: '50%',
                    })}
                  ></workbench-template-loading>`
                : repeat(
                    _templates,
                      ...
                      return html`
                        ...
                        >
                          ...
                          ${template === this._loadingTemplate
                            ? html`<workbench-template-loading></workbench-template-loading>`
                            : nothing}
                          ${template.name
                            ? html`<workbench-tooltip
                                .offset=${12}
                                tip-position="top"
                              >
                                ${template.name}
