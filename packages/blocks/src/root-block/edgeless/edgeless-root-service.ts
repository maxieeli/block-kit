...
import { type BlockModel, Slot } from '@maxiee/block_store';
...
export class EdgelessRootService extends RootService {
  ...
  addBlock(
    ...
    this.editPropsStore.apply(
      flavour as BlockKit.EdgelessModelKeyType,
      props
    );
  ...
  updateElement(id: string, props: Record<string, unknown>) {
    ...
    if (element) {
      this.editPropsStore.record(
        element.type as BlockKit.EdgelessModelKeyType,
        props
      );
    ...
    if (block) {
      this.editPropsStore.record(
        block.flavour as BlockKit.EdgelessModelKeyType,
        props
      );
  ...
  removeElement(id: string | BlockKit.EdgelessModelType) {
  ...
  getElementById(id: string): BlockKit.EdgelessModelType | null {
    const el =
      this._surface.getElementById(id) ??
      (this.doc.getBlockById(id) as BlockKit.EdgelessBlockModelType | null);
    return el;
  }
  ...
  ): BlockKit.EdgelessModelType | null;
  pickElement(
    x: number,
    y: number,
    options: IHitTestOptions = { all: false, expand: 10 }
  ): BlockKit.EdgelessModelType[] | BlockKit.EdgelessModelType | null {
    options.expand ??= 10;
    ...
    const pickCanvasElement = () => {
      ...
      return picked as BlockKit.EdgelessModelType[];
    };
    const pickBlock = () => {
  ...
  pickElementsByBound(
    bound: IBound | Bound,
    type: 'canvas'
  ): BlockKit.SurfaceElementModelType[];
  pickElementsByBound(
    bound: IBound | Bound,
  ...
  pickElementInGroup(
    ...
    return (picked ?? first) as BlockKit.EdgelessModelType | null;
  }
  ...
  createGroup(elements: BlockKit.EdgelessModelType[] | string[]) {
  ...
  getConnectors(element: BlockKit.EdgelessModelType | string) {
    ...
    if (type === 'sticker') {
      middlewares.push(
        createStickerMiddleware(this.viewport.center, () =>
          this.layer.generateIndex('workbench:image')
        )
      );
    }
