...
export class RootService extends BlockService<RootBlockModel> {
  ...
  private _getParentModelBySelection = (): {
    ...
    if (currentMode === 'edgeless') {
      const surface =
        root.children.find(child => child.flavour === 'workbench:surface') ?? null;
    ...
    if (currentMode === 'page') {
      ...
      while (selectedBlock && selectedBlock.flavour !== 'workbench:note') {
  ...
  private _insertLink = (url: string) => {
    const host = this.host as EditorHost;
    const rootService = host.spec.getService('workbench:page');

    const embedOptions = rootService.getEmbedBlockOptions(url);

    let flavour = 'workbench:bookmark';
  ...
  private _insertDoc = (docId: string) => {
    const flavour = 'workbench:embed-linked-doc';
  ...
  appendParagraph = (text: string = '') => {
    ...
    if (!noteId) {
      noteId = doc.addBlock('workbench:note', {}, doc.root.id);
    }
  ...
  > => {
    ...
    if ('docId' in result) {
      ...
      return { flavour: 'workbench:embed-linked-doc', isNewDoc: result.isNewDoc };
    }
