...
import type { MigrationRunner, Y } from '@maxiee/block_store';
import {
  BlockModel,
  Boxed,
  defineBlockSchema,
...
export class SurfaceBlockModel extends BlockModel<SurfaceBlockProps> {
  private _elementModels = new Map<
    string,
    {
      mount: () => void;
      unmount: () => void;
      model: BlockKit.SurfaceElementModelType;
    }
  >();
  ...
  get elementModels() {
    const models: BlockKit.SurfaceElementModelType[] = [];
  ...
  private _watchGroupRelationChange() {
    ...
    const isGroup = (
      element: BlockKit.SurfaceElementModelType
    ): element is BlockKit.SurfaceGroupLikeModelType =>
  ...
  getElementById(id: string): BlockKit.SurfaceElementModelType | null {
  ...
  removeElement(id: string) {
    ...
    this.doc.transact(() => {
      ...
      this.hooks.remove.emit({
        id,
        model: element as BlockKit.SurfaceElementModelType,
        type: element.type,
      });
    });
  }
