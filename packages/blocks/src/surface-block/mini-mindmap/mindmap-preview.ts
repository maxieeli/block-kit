import { type EditorHost, WithDisposable } from '@maxiee/block-std';
import { noop } from '@maxiee/block_global/utils';
import { type Doc, Job } from '@maxiee/block_store';
import {
  DocCollection,
...
export class MiniMindmapPreview extends WithDisposable(LitElement) {
  static override styles = css`
    ...
    .template-item {
      ...
      border: 2px solid var(--workbench-border-color);
      border-radius: 4px;
      padding: 4px 6px;
    }
    ...
    .template-item:hover {
      border-color: var(--workbench-brand-color);
    }
  ...
  private _createTemporaryDoc() {
    ...
    const surfaceId = doc.addBlock('workbench:surface', {}, rootId);
