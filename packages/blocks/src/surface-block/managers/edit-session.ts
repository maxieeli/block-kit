import type { BlockService } from '@maxiee/block-std';
...
const SESSION_PROP_KEY = 'blockkit:prop:record';
...
export class EditPropsStore {
  ...
  private _getKey<T extends keyof StorageProps>(key: T) {
    ...
    switch (key) {
      case 'viewport':
        return 'blockkit:' + id + ':edgelessViewport';
      case 'presentBlackBackground':
        return 'blockkit:presentation:blackBackground';
      case 'presentFillScreen':
        return 'blockkit:presentation:fillScreen';
      case 'presentHideToolbar':
        return 'blockkit:presentation:hideToolbar';
      case 'templateCache':
        return 'blockkit:' + id + ':templateTool';
      case 'remoteColor':
        return 'blockkit:remote-color';
      case 'showBidirectional':
        return 'blockkit:' + id + ':showBidirectional';
      case 'autoHideEmbedHTMLFullScreenToolbar':
        return 'blockkit:embedHTML:autoHideFullScreenToolbar';
      default:
        return key;
    }
  }
  ...
  record(
    type: BlockKit.EdgelessModelKeyType,
  ...
  apply(type: BlockKit.EdgelessModelKeyType, props: Record<string, unknown>) {
    if (!isLastPropType(type)) return;
