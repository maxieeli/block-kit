import { assertEquals, assertExists } from '@maxiee/block_global/utils';
...
function rBound(ele: BlockKit.EdgelessModelType, anti = false): IBound {
...
export function getAnchors(ele: BlockKit.EdgelessModelType) {
  const bound = Bound.deserialize(ele.xywh);
  const offset = 10;
...
function getConnectableRelativePosition(
  connectable: BlockKit.EdgelessModelType,
  position: IVec
) {
