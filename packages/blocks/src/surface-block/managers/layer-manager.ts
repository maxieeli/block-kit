import { assertType, DisposableGroup, Slot } from '@maxiee/block_global/utils';
...
export class LayerManager {
  ...
  slots = {
    layerUpdated: new Slot<{
      ...
      initiatingElement: BlockKit.EdgelessModelType;
    }>(),
  };
  ...
  constructor(elements?: BlockKit.EdgelessModelType[]) {
    if (elements) {
      this._init(elements);
    }
  }
  ...
  private _init(elements: BlockKit.EdgelessModelType[]) {
    this.canvasElements = [];
    this.blocks = [];
    this.frames = [];
    ...
    elements.forEach(element => {
      ...
      } else if (matchFlavours(element, ['workbench:frame'])) {
  ...
  private _insertIntoLayer(
    target: BlockKit.EdgelessModelType,
    type: 'block' | 'canvas'
  ) {
    ...
    const createLayer = (
      type: 'block' | 'canvas',
      targets: BlockKit.EdgelessModelType[],
      curZIndex: number
    ): Layer => {
      const newLayer = {
        type,
  ...
  private _updateLayer(
    ...
    const updateArray = (
      array: BlockKit.EdgelessModelType[],
      element: BlockKit.EdgelessModelType
    ) => {
    ...
    } else if (matchFlavours(element as BlockModel, ['workbench:frame'])) {
    ...
    if (updateType && (indexChanged || childIdsChanged)) {
      this._removeFromLayer(
        element as BlockKit.EdgelessModelType,
        updateType
      );
  ...
  add(element: BlockKit.EdgelessModelType) {
  ...
  delete(element: BlockKit.EdgelessModelType) {
  ...
  generateIndex(elementType: string): string {
    const batch = elementType === 'workbench:frame' ? 'frame' : 'common';
    const type = elementType.startsWith('workbench:') ? 'block' : 'canvas';
  ...
  createIndexGenerator(ignoreRule: boolean = false) {
    ...
    return (elementType: string) => {
      if (ignoreRule && elementType !== 'workbench:frame') {
        elementType = 'shape';
      }
      ...
      manager.add(mockedFakeElement as unknown as BlockKit.EdgelessModelType);
  ...
  getReorderedIndex(
    ...
    const isFrameBlock =
      (element as FrameBlockModel).flavour === 'workbench:frame';

    let elements: BlockKit.EdgelessModelType[];
    ...
    if (group !== null) {
      ...
      elements = this.layers.reduce(
        (pre: BlockKit.EdgelessModelType[], current) =>
   ...
   */
  compare(a: BlockKit.EdgelessModelType, b: BlockKit.EdgelessModelType) {
    return compare(a, b);
  }
