...
export class KeymapController implements ReactiveController {
  ...
  private _onBlockShiftDown = (cmd: BlockKit.CommandChain) => {
    return cmd
      .getBlockSelections()
  ...
  private _onBlockShiftUp = (cmd: BlockKit.CommandChain) => {
    return cmd
      .getBlockSelections()
      .inline<'currentSelectionPath' | 'anchorBlock'>((ctx, next) => {
        const blockSelections = ctx.currentBlockSelections;
  ...
  private _onEnter = (ctx: UIEventStateContext) => {
    ...
    const [result] = this._std.command
      ...
      .inline((ctx, next) => {
        ...
        const blockId = doc.addBlock('workbench:paragraph', {}, parent, index + 1);
          ...
          [key]: ctx => {
            ...
            const [result] = this._std.command
              ...
              .inline((ctx, next) => {
                ...
                if (item.flavour !== 'workbench:code') {
                  return;
                }
