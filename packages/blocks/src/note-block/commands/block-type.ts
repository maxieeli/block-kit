...
> = (ctx, next) => {
  ...
  const appendDivider: Command<never, 'updatedBlocks'> = (_, next) => {
    ...
    const id = doc.addBlock('workbench:divider', {}, parent, index + 1);
    if (!nextSibling) {
      nextSiblingId = doc.addBlock('workbench:paragraph', {}, parent);
    }
  ...
  const [result, resultCtx] = std.command
    ...
    .try(chain => [
      chain.inline((_, next) => {
        if (['workbench:code', 'workbench:divider'].includes(flavour)) {
          return next();
        }
        return false;
      }),
