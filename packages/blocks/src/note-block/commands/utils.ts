...
function isActive(std: BlockKit.Std, key: keyof WorkbenchTextAttributes) {
...
function getCombinedFormatFromInlineEditors(
  inlineEditors: [WorkbenchInlineEditor, InlineRange | null][]
): WorkbenchTextAttributes {
  const formatArr: WorkbenchTextAttributes[] = [];
  ...
  return formatArr.reduce((acc, cur) => {
    const newFormat: WorkbenchTextAttributes = {};
    for (const key in acc) {
      const typedKey = key as keyof WorkbenchTextAttributes;
...
function getSelectedInlineEditors(
  blocks: BlockElement[],
  filter: (
    inlineRoot: InlineRootElement<WorkbenchTextAttributes>
  ) => InlineEditor<WorkbenchTextAttributes> | []
) {
...
function handleCurrentSelection<
  InlineOut extends BlockKit.CommandDataName = never,
>(
  chain: Chain<InitCommandCtx>,
  handler: (
    type: 'text' | 'block' | 'native',
    inlineEditors: InlineEditor<WorkbenchTextAttributes>[]
  ...
  return chain.try<InlineOut>(chain => [
    ...
    chain.inline<InlineOut>((ctx, next) => {
      const selectedInlineEditors = Array.from<InlineRootElement>(
        ...
        .map((el): WorkbenchInlineEditor => el.inlineEditor);
