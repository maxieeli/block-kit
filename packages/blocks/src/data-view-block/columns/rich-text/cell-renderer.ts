import { IS_MAC } from '@maxiee/block_global/env';
import { assertExists } from '@maxiee/block_global/utils';
import type { Y } from '@maxiee/block_store';
import { DocCollection, Text } from '@maxiee/block_store';
...
import type {
  WorkbenchInlineEditor,
  WorkbenchTextAttributes,
...
function toggleStyle(
  ...
  let oldAttributes: WorkbenchTextAttributes = {};
...
@customElement('workbench-data-view-rich-text-cell')
export class RichTextCell extends BaseCellRenderer<Y.Text> {
  get service() {
    const database = this.closest<DatabaseBlockComponent>('workbench-data-view');
  ...
  get topContenteditableElement() {
    const databaseBlock =
      this.closest<DatabaseBlockComponent>('workbench-data-view');
    return databaseBlock?.topContenteditableElement;
  }

  static override styles = css`
    workbench-data-view-rich-text-cell {
      display: flex;
      align-items: center;
      width: 100%;
      user-select: none;
    ...
    .workbench-data-view-rich-text v-line {
    ...
    .workbench-data-view-rich-text v-line > div {
      flex-grow: 1;
    }
  `;
  ...
  override render() {
    ...
    if (!this.value) {
      return html`<div class="workbench-data-view-rich-text"></div>`;
    }
    return html`<rich-text
      ...
      .readonly=${true}
      class="workbench-data-view-rich-text inline-editor"
    ></rich-text>`;
  }
}

@customElement('workbench-data-view-rich-text-cell-editing')
export class RichTextCellEditing extends BaseCellRenderer<Text> {
  ...
  static override styles = css`
    workbench-data-view-rich-text-cell-editing {
      display: flex;
      align-items: center;
      width: 100%;
      min-width: 1px;
...
declare global {
  interface HTMLElementTagNameMap {
    'workbench-data-view-rich-text-cell-editing': RichTextCellEditing;
  }
}
