...
export class FrameBlockComponent extends BlockElement<FrameBlockModel> {
  ...
  get titleElement(): EdgelessFrameTitle | null {
    return (
      this.closest('workbench-edgeless-root')?.querySelector?.(
  ...
  private get _surface() {
    return this.closest('workbench-edgeless-root')!.surface;
  }

  private get _edgeless() {
    return this.closest('workbench-edgeless-root');
  }
  ...
  override renderBlock() {
    ...
    return html`
      <div
        class="workbench-frame-container"
        style=${styleMap({
          width: bound.w + 'px',
          height: bound.h + 'px',
          backgroundColor: isCssVariable(model.background)
            ...
            : '',
          borderRadius: '8px',
          border: _isNavigator ? 'none' : `2px solid var(--workbench-black-30)`,
        })}
      ></div>
    `;
  }
...
declare global {
  interface HTMLElementTagNameMap {
    'workbench-frame': FrameBlockComponent;
  }
}
