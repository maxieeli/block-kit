...
import { DocCollection } from '@maxiee/block_store';
...
@customElement('workbench-embed-synced-doc-block')
@Peekable()
...
> {
  ...
  @query(
    ':scope > .workbench-block-component > .embed-block-container > workbench-embed-synced-doc-card'
  )
  accessor syncedDocCard: EmbedSyncedDocCard | null = null;

  @query(
    ':scope > .workbench-block-component > .embed-block-container > .workbench-embed-synced-doc-container > .workbench-embed-synced-doc-editor > div > editor-host'
  )
  ...
  private _renderSyncedView = () => {
    ...
    const renderEditor = () => {
      return choose(editorMode, [
        [
          'page',
          () => html`
            <div class="workbench-page-viewport">
        ...
        [
          'edgeless',
          () => html`
            <div class="workbench-edgeless-viewport">
              ${this.host.renderSpecPortal(
                syncedDoc,
    ...
    return this.renderEmbed(
      () => html`
        <div
          class=${classMap({
            'workbench-embed-synced-doc-container': true,
            [editorMode]: true,
            [theme]: true,
            selected: isSelected,
            surface: isInSurface,
          ...
          ?data-scale=${scale}
        >
          <div class="workbench-embed-synced-doc-editor">
            ${this.isPageMode && this._isEmptySyncedDoc
              ? html`
                  <div class="workbench-embed-synced-doc-editor-empty">
                    <span>
          ...
          ${isInSurface
            ...
            : html`
                <div
                  class=${classMap({
                    'workbench-embed-synced-doc-header-wrapper': true,
                    selected: isSelected,
                  })}
                >
                  <div class="workbench-embed-synced-doc-header">
                    ${icon}
                    <span class="workbench-embed-synced-doc-title">
                      ${this.docTitle}
                    </span>
                  </div>
                </div>
              `}
          ${isInSurface
            ? html` <div class="workbench-embed-synced-doc-editor-overlay"></div> `
            : nothing}
        </div>
      `
    );
  ...
  override firstUpdated() {
    ...
    const syncedDocRootService =
      this.syncedDocEditorHost?.std.spec.getService('workbench:page');
