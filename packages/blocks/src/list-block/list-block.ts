...
> {
  ...
  private _onClickIcon = (e: MouseEvent) => {
    ...
    if (this.model.type === 'toggle') {
      ...
      if (this.model.checked) {
        const checkEl = this.querySelector('.workbench-list-block__todo-prefix');
  ...
  private _updateFollowingListSiblings() {
    this.updateComplete
      .then(() => {
        ...
        while (current?.tagName == 'WORKBENCH-LIST') {
  ...
  override renderBlock(): TemplateResult<1> {
    ...
    return html`
      <div class=${'workbench-list-block-container'}>
        <div class=${`workbench-list-rich-text-wrapper ${checked}`}>
...
declare global {
  interface HTMLElementTagNameMap {
    'workbench-list': ListBlockComponent;
  }
}
