...
const styles = css`
  .workbench-database-toolbar-item.new-record {
    margin-left: 12px;
    display: flex;
    align-items: center;
    gap: 4px;
...
export class DataViewHeaderToolsAddRow extends WidgetBase {
  ...
  _dragStart = (e: MouseEvent) => {
    e.preventDefault();
    e.stopPropagation();
    const container = this.closest('workbench-data-view-renderer');
    const tableRect = container
      ?.querySelector('workbench-database-table')
      ?.getBoundingClientRect();
    const rows: NodeListOf<HTMLElement> | undefined =
      container?.querySelectorAll('.workbench-database-block-row');
    if (!rows || !tableRect) {
      return;
    }
  ...
  override render() {
    ...
    return html` <div
      class="workbench-database-toolbar-item new-record"
      draggable="true"
