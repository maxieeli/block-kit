...
const styles = css`
  .workbench-database-filter-button {
    display: flex;
    align-items: center;
    gap: 6px;
    font-size: 12px;
  ...
  .workbench-database-filter-button:hover {
    background-color: var(--workbench-hover-color);
  }

  .workbench-database-filter-button svg {
    width: 20px;
    height: 20px;
  }
`;
...
export class DataViewHeaderToolsFilter extends WidgetBase {
  ...
  override render() {
    ...
    return html`<div
      ...
      class="workbench-database-filter-button dv-icon-20"
    >
      ${FilterIcon} Filter
    </div>`;
  }
