...
export class DetailSelection {
  ...
  getSelectCard(selection: KanbanCardSelection) {
    ...
    return this.viewEle
      .querySelector(`workbench-data-view-kanban-group[data-key="${groupKey}"]`)
      ?.querySelector(
        `workbench-data-view-kanban-card[data-card-id="${cardId}"]`
      ) as KanbanCard | undefined;
  }

  getFocusCellContainer(selection: DetailViewSelection) {
    return this.viewEle.querySelector(
      `workbench-data-view-record-field[data-column-id="${selection.propertyId}"]`
  ...
  focusFirstCell() {
    const firstId = this.viewEle.querySelector('workbench-data-view-record-field')
      ?.column.id;
    if (firstId) {
      this.selection = {
        propertyId: firstId,
