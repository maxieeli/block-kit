...
const styles = css`
  ${unsafeCSS(dataViewCommonStyle('workbench-data-view-record-detail'))}
  workbench-data-view-record-detail {
    display: flex;
    flex: 1;
...
@customElement('workbench-data-view-record-detail')
export class RecordDetail extends WithDisposable(ShadowlessElement) {
  ...
  override connectedCallback() {
    ...
    this.dataset.widgetId = 'workbench-detail-widget';
  ...
  override render() {
    ...
    return html`
      ...
      ${repeat(
        ...
        v => v.id,
        column => {
          return html` <workbench-data-view-record-field
            .view="${this.view}"
            .column="${column}"
            .rowId="${this.rowId}"
...
declare global {
  interface HTMLElementTagNameMap {
    'workbench-data-view-record-detail': RecordDetail;
  }
}
