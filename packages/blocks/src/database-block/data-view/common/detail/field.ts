...
@customElement('workbench-data-view-record-field')
export class RecordField extends WithDisposable(ShadowlessElement) {
  ...
  static override styles = css`
    workbench-data-view-record-field {
      display: flex;
      gap: 12px;
    }
    ...
    workbench-data-view-record-field .icon {
      display: flex;
      align-items: center;
      width: 16px;
      height: 16px;
    ...
    workbench-data-view-record-field .icon svg {
      width: 16px;
      height: 16px;
    ...
    .field-content .workbench-database-number {
      text-align: left;
  ...
  changeEditing = (editing: boolean) => {
    const selection = this.closest('workbench-data-view-record-detail')?.selection;
    if (selection) {
      selection.selection = {
...
declare global {
  interface HTMLElementTagNameMap {
    'workbench-data-view-record-field': RecordField;
  }
}
