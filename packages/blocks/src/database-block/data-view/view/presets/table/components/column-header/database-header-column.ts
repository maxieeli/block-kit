...
@customElement('workbench-database-header-column')
export class DatabaseHeaderColumn extends WithDisposable(ShadowlessElement) {
  ...
  static override styles = css`
    ...
    .workbench-database-header-column-grabbing * {
      cursor: grabbing;
    }
  `;
  ...
  private _columnsOffset = (header: Element, _scale: number) => {
    const columns = header.querySelectorAll('workbench-database-header-column');
  ...
  private moveColumn = (evt: PointerEvent) => {
    ...
    const headerContainer = this.closest('workbench-database-column-header');
    ...
    html?.classList.toggle('workbench-database-header-column-grabbing', true);
    const drag = startDrag<{
      ...
      onClear: () => {
        cancelScroll();
        html?.classList.toggle('workbench-database-header-column-grabbing', false);
        dropPreview.remove();
        dragPreview.remove();
      },
    });
  ...
  override connectedCallback() {
    ...
    const table = this.closest('workbench-database-table');
    if (table) {
      this.disposables.add(
  ...
  override render() {
    ...
    const classes = classMap({
      'workbench-database-column-move': true,
      [this.grabStatus]: true,
    });
    return html`
      <div
        style=${style}
        class="workbench-database-column-content"
      ...
      >
        ...
        <div class="workbench-database-column-text ${column.type}">
          <div
            class="workbench-database-column-type-icon dv-hover"
            @click="${this._clickTypeIcon}"
          >
            <uni-lit .uni="${column.icon}"></uni-lit>
          </div>
          <div class="workbench-database-column-text-content">
            <div class="workbench-database-column-text-input">${column.name}</div>
          </div>
        </div>
      </div>
      <div
...
declare global {
  interface HTMLElementTagNameMap {
    'workbench-database-header-column': DatabaseHeaderColumn;
  }
}
