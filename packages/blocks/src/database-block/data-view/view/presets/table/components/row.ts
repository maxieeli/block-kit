...
export class TableRow extends WithDisposable(ShadowlessElement) {
  get selectionController() {
    return this.closest('workbench-database-table')?.selectionController;
  }

  get groupKey() {
    return this.closest('workbench-data-view-table-group')?.group?.key;
  }
  ...
  static override styles = css`
    ...
    .workbench-database-block-row.selected > .database-cell {
    ...
    .workbench-database-block-row:hover .row-ops {
      visibility: visible;
    }
    .workbench-database-block-row .show-on-hover-row {
      visibility: hidden;
      opacity: 0;
      transition: opacity 150ms cubic-bezier(0.42, 0, 1, 1);
    }
    .workbench-database-block-row:hover .show-on-hover-row {
      visibility: visible;
      opacity: 1;
    }
      ...
      <div class="data-view-table-left-bar">
        ...
        >
          <div
            ...
            border-radius: 2px;
            height: 12px;
            background-color: var(--workbench-placeholder-color);"
          ></div>
        </div>
      </div>
      ${repeat(
  ...
  contextMenu = (e: MouseEvent) => {
    ...
    const cell = ele.closest('workbench-database-cell-container');
  ...
  override connectedCallback() {
    ...
    this.classList.add('workbench-database-block-row', 'database-row');
  }
}
