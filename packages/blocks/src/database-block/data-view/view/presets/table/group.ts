...
const styles = css`
  workbench-data-view-table-group .group-header-op {
    visibility: visible;
  }
...
@customElement('workbench-data-view-table-group')
export class TableGroup extends WithDisposable(ShadowlessElement) {
  ...
  private renderRows(ids: string[]) {
    return html`
      ...
      ></workbench-database-column-header>
      <div class="workbench-database-block-rows">
        ${repeat(
          ids,
          id => id,
          (id, idx) => {
        ...
        : html` <div
          ...
          >
            <div
              ...
              data-test-id="workbench-database-add-row-button"
              role="button"
            >
      ...
      ${this.dataViewEle.config.getFlag?.('enable_database_statistics')
        ? html`
            <workbench-database-column-stats .view="${this.view}">
...
declare global {
  interface HTMLElementTagNameMap {
    'workbench-data-view-table-group': TableGroup;
  }
}
