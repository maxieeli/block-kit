...
const styles = css`
  ...
  .workbench-database-block-title-container {
    display: flex;
    align-items: center;
  ...
  .workbench-database-block-table {
    position: relative;
    width: 100%;
    padding-bottom: 4px;
    z-index: 1;
  ...
  .workbench-database-block-table:hover {
    padding-bottom: 0px;
  }

  .workbench-database-block-table::-webkit-scrollbar {
    -webkit-appearance: none;
    display: block;
  }

  .workbench-database-block-table::-webkit-scrollbar:horizontal {
    height: 4px;
  }

  .workbench-database-block-table::-webkit-scrollbar-thumb {
    border-radius: 2px;
    background-color: transparent;
  }

  .workbench-database-block-table:hover::-webkit-scrollbar:horizontal {
    height: 8px;
  }

  .workbench-database-block-table:hover::-webkit-scrollbar-thumb {
    border-radius: 16px;
    background-color: var(--workbench-black-30);
  }

  .workbench-database-block-table:hover::-webkit-scrollbar-track {
    background-color: var(--workbench-hover-color);
  }

  .workbench-database-table-container {
    position: relative;
    width: fit-content;
    min-width: 100%;
  }

  .workbench-database-block-tag-circle {
    width: 12px;
    height: 12px;
    border-radius: 50%;
...
@customElement('workbench-database-table')
...
> {
      ...
      return html`
        <div style="display:flex;flex-direction: column;gap: 16px;">
          ${groupHelper.groups.map(group => {
            return html`<workbench-data-view-table-group
              data-group-key="${group.key}"
              .dataViewEle="${this.dataViewEle}"
              .view="${this.view}"
              .viewEle="${this}"
              .group="${group}"
            ></workbench-data-view-table-group>`;
          })}
  ...
  override connectedCallback() {
    ...
    this._disposables.add(
      this.view.slots.update.on(() => {
        this.requestUpdate();
        this.querySelectorAll('workbench-data-view-table-group').forEach(v => {
          v.requestUpdate();
        });
      })
    );
  ...
  renderAddGroup = (groupHelper: GroupHelper) => {
    ...
    return html` <div style="display:flex;">
      <div
        ...
        style="display:flex;align-items:center;gap: 10px;padding: 6px 12px 6px 8px;color: var(--workbench-text-secondary-color);font-size: 12px;line-height: 20px;position: sticky;left: ${LEFT_TOOL_BAR_WIDTH}px;"
        @click="${add}"
      >
  ...
  override render() {
    return html`
      ...
      <div class="workbench-database-table">
        <div class="workbench-database-block-table" @wheel="${this.onWheel}">
          <div class="workbench-database-table-container">
            ${this.renderTable()}
          </div>
        </div>
      </div>
...
declare global {
  interface HTMLElementTagNameMap {
    'workbench-database-table': DataViewTable;
  }
}
