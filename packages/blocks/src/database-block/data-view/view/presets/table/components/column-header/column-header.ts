...
@customElement('workbench-database-column-header')
export class DatabaseColumnHeader extends WithDisposable(ShadowlessElement) {
  ...
  override connectedCallback() {
    ...
    const scrollContainer = this.getScrollContainer(
      this.closest('workbench-data-view-renderer')!
    );
    const group = this.closest('workbench-data-view-table-group');
    if (group) {
    ...
    if (group) {
      ...
      this.closest('workbench-data-view-renderer')?.append(this.addColumnButton);
      requestAnimationFrame(() => {
  ...
  updateAddButton = () => {
    ...
    computePosition(referenceEl, this.addColumnButton, {
      middleware: [
        shift({
          boundary: this.closest('workbench-database-table') ?? this,
          padding: -20,
        }),
      ],
    })
  ...
  editLastColumnTitle = () => {
    const columns = this.querySelectorAll('workbench-database-header-column');
  ...
  override render() {
    ...
    return html`
      ...
      <div class="workbench-database-column-header database-row">
        ...
        ${repeat(
          ...
          (column, index) => {
            ...
            return html` <workbench-database-header-column
              style="${style}"
              data-column-id="${column.id}"
              data-column-index="${index}"
              class="workbench-database-column database-cell"
              .column="${column}"
        ...
        <div
          style="background-color: var(--workbench-border-color);width: 1px;"
        ></div>
...
declare global {
  interface HTMLElementTagNameMap {
    'workbench-database-column-header': DatabaseColumnHeader;
  }
}
