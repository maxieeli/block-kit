...
export class TableSelectionController implements ReactiveController {
  ...
  private resolveDragStartTarget(
    ...
    if (fillValues) {
      ...
      cell = target.closest('workbench-database-cell-container');
    }
  ...
  cellPosition(groupKey: string | undefined) {
    ...
    const cells = rows
      ?.item(0)
      .querySelectorAll('workbench-database-cell-container');
  ...
  startDrag(
    ...
    const groupKey = cell.closest('workbench-data-view-table-group')?.group?.key;
  ...
  focusToCell(position: 'left' | 'right' | 'up' | 'down') {
    ...
    const rows = Array.from(
      row
        ?.closest('.workbench-database-table-container')
        ?.querySelectorAll('data-view-table-row') ?? []
    );
    const cells = Array.from(
      row?.querySelectorAll('workbench-database-cell-container') ?? []
    );
    if (!row || !rows || !cells) {
      return;
    }
    ...
    const container =
      groupKey != null
        ? this.tableContainer.querySelector(
            `workbench-data-view-table-group[data-group-key="${groupKey}"]`
          )
        : this.tableContainer;
  ...
  updateFocusSelectionStyle(
    ...
    if (focus && !isRowSelection) {
      ...
      div.style.borderColor = 'var(--workbench-primary-color)';
  ...
  getRect(
    ...
    const topCells = topRow.querySelectorAll('workbench-database-cell-container');
...
class SelectionElement extends ShadowlessElement {
  static override styles = css`
    .database-selection {
      ...
      background: var(--workbench-primary-color-04);
      pointer-events: none;
      display: none;
    }
