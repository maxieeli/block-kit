...
const styles = css`
  workbench-data-view-kanban-card {
    display: flex;
    position: relative;
    ...
    background-color: var(--workbench-background-kanban-card-color);
  }

  workbench-data-view-kanban-card:hover {
    background-color: var(--workbench-hover-color);
  }

  workbench-data-view-kanban-card .card-header {
    padding: 8px;
    display: flex;
    flex-direction: column;
    gap: 8px;
  ...
  workbench-data-view-kanban-card .card-header-title uni-lit {
    width: 100%;
  }

  .card-header.has-divider {
    border-bottom: 0.5px solid var(--workbench-border-color);
  }

  workbench-data-view-kanban-card .card-header-title {
    font-size: var(--data-view-cell-text-size);
    line-height: var(--data-view-cell-text-line-height);
  }

  workbench-data-view-kanban-card .card-header-icon {
    padding: 4px;
    background-color: var(--workbench-background-secondary-color);
    display: flex;
    align-items: center;
    border-radius: 4px;
    width: max-content;
  ...
  workbench-data-view-kanban-card .card-header-icon svg {
    width: 16px;
    height: 16px;
  ...
  workbench-data-view-kanban-card .card-body {
    display: flex;
    flex-direction: column;
    padding: 8px;
    gap: 4px;
  ...
  workbench-data-view-kanban-card:hover .card-ops {
    visibility: visible;
  }
...
@customElement('workbench-data-view-kanban-card')
export class KanbanCard extends WithDisposable(ShadowlessElement) {
  ...
  private renderTitle() {
    ...
    return html` <div class="card-header-title">
      <workbench-data-view-kanban-cell
        .contentOnly="${true}"
    ...
    return html` <div class="card-body">
      ${repeat(
        ...
        column => {
          ...
          return html` <workbench-data-view-kanban-cell
            .contentOnly="${false}"
  ...
  private getSelection() {
    return this.closest('workbench-data-view-kanban')?.selectionController;
  }
  ...
  override render() {
    ...
    this.style.border = this.isFocus
      ? '1px solid var(--workbench-primary-color)'
      : '';
    return html`
...
declare global {
  interface HTMLElementTagNameMap {
    'workbench-data-view-kanban-card': KanbanCard;
  }
}
