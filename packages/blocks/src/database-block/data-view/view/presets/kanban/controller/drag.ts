...
export class KanbanDragController implements ReactiveController {
    ...
    this.host.disposables.add(
      this.host.handleEvent('dragStart', context => {
        ...
        if (target instanceof Element) {
          const cell = target.closest('workbench-data-view-kanban-cell');
          if (cell?.editing) {
            return;
          }
          cell?.selectCurrentCell(false);
          const card = target.closest('workbench-data-view-kanban-card');
          if (card) {
  ...
  dragStart = (ele: KanbanCard, evt: PointerEvent) => {
    ...
    const currentGroup = ele.closest('workbench-data-view-kanban-group');
...
const createDragPreview = (card: KanbanCard, x: number, y: number) => {
  ...
  kanbanCard.style.backgroundColor = 'var(--workbench-background-primary-color)';
...
const createDropPreview = () => {
  ...
  div.style.backgroundColor = 'var(--workbench-primary-color)';
