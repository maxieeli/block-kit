...
const styles = css`
  ...
  workbench-data-view-kanban-group:hover .group-header-op {
    visibility: visible;
    opacity: 1;
  }
  ...
  workbench-data-view-kanban-group:hover .add-card {
    visibility: visible;
    opacity: 1;
  }

  workbench-data-view-kanban-group .add-card:hover {
    background-color: var(--workbench-hover-color);
    color: var(--workbench-text-primary-color);
  }

  workbench-data-view-kanban-group:has(.add-card:hover) .add-card {
    background-color: var(--workbench-hover-color);
    color: var(--workbench-text-primary-color);
  }
...
@customElement('workbench-data-view-kanban-group')
export class KanbanGroup extends WithDisposable(ShadowlessElement) {
  ...
  private clickAddCard = () => {
    ...
    requestAnimationFrame(() => {
      const kanban = this.closest('workbench-data-view-kanban');
      if (kanban) {
...
declare global {
  interface HTMLElementTagNameMap {
    'workbench-data-view-kanban-group': KanbanGroup;
  }
}
