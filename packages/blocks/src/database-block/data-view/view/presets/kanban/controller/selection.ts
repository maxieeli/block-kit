...
export class KanbanSelectionController implements ReactiveController {
  ...
  shiftClickCard = (event: MouseEvent) => {
    ...
    const closestGroupKey = target.closest('workbench-data-view-kanban-group')
      ?.group.key;
  ...
  getNextFocusCell(
    ...
    const group = this.host.querySelector(
      `workbench-data-view-kanban-group[data-key="${selection.groupKey}"]`
    );
    const cards = Array.from(
      group?.querySelectorAll('workbench-data-view-kanban-card') ?? []
    );
    ...
    const groups = Array.from(
      this.host.querySelectorAll('workbench-data-view-kanban-group')
    );
  ...
  getNextFocusCard(
    ...
    const group = this.host.querySelector(
      `workbench-data-view-kanban-group[data-key="${selection.cards[0].groupKey}"]`
    );
  ...
  focusIn() {
    ...
    if (selection.selectionType === 'card') {
      ...
      const cell = card?.querySelector('workbench-data-view-kanban-cell');
      if (cell) {
        this.selection = {
          groupKey: card.groupKey,
          cardId: card.cardId,
...
function getNextGroupFocusElement(
  ...
  const nextCards = Array.from(
    nextGroup.querySelectorAll('workbench-data-view-kanban-card')
  );
  ...
  const cells = Array.from(
    nextCard.querySelectorAll('workbench-data-view-kanban-cell')
  );
  const cellPos = cells
    .map((card, index) => {
      const targetRect = card.getBoundingClientRect();
...
function getCardCellsBySelection(
  ...
  return Array.from(
    card?.querySelectorAll('workbench-data-view-kanban-cell') ?? []
  );
}
...
function getSelectedCard(
  ...
  return group.querySelector<KanbanCard>(
    `workbench-data-view-kanban-card[data-card-id="${selection.cardId}"]`
  );
}
  ...
  const groups = groupKeys
    .map(key =>
      viewElement.querySelector(
        `workbench-data-view-kanban-group[data-key="${key}"]`
      )
    )
    ...
    .flatMap(group =>
      cardIds.map(id =>
        group.querySelector<KanbanCard>(
          `workbench-data-view-kanban-card[data-card-id="${id}"]`
        )
      )
    )
...
function getFocusCell(viewElement: Element, selection: KanbanCellSelection) {
  ...
  return card?.querySelector<KanbanCell>(
    `workbench-data-view-kanban-cell[data-column-id="${selection.columnId}"]`
  );
}
