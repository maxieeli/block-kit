...
@customElement('workbench-database-number-cell')
export class NumberCell extends BaseCellRenderer<number> {
  static override styles = css`
    workbench-database-number-cell {
      display: block;
      width: 100%;
    }

    .workbench-database-number {
      display: flex;
      align-items: center;
  ...
  override render() {
    return html` <div class="workbench-database-number number">
      ${this.value ?? ''}
    </div>`;
  }
}

@customElement('workbench-database-number-cell-editing')
export class NumberCellEditing extends BaseCellRenderer<number> {
  static override styles = css`
    workbench-database-number-cell-editing {
      display: block;
      width: 100%;
      cursor: text;
    }
    ...
    .workbench-database-number:focus {
      outline: none;
    }
  `;
  ...
  override render() {
    ...
    return html`<input
      ...
      @blur="${this._blur}"
      @focus="${this._focus}"
      class="workbench-database-number number"
