...
@customElement('workbench-database-date-cell')
export class DateCell extends BaseCellRenderer<number> {
  static override styles = css`
    workbench-database-date-cell {
      width: 100%;
    }

    .workbench-database-date {
      display: flex;
      align-items: center;
      width: 100%;
      padding: 0;
    ...
    input.workbench-database-date[type='date']::-webkit-calendar-picker-indicator {
      display: none;
    }
  `;
  ...
  override render() {
    ...
    return html` <input
      type="date"
      value="${value}"
      class="workbench-database-date date"
    />`;
  }
}

@customElement('workbench-database-date-cell-editing')
export class DateCellEditing extends BaseCellRenderer<number> {
  static override styles = css`
    workbench-database-date-cell-editing {
      width: 100%;
      cursor: text;
    }

    .workbench-database-date:focus {
      outline: none;
    }
  `;
  ...
  private _onFocus = () => {
    ...
    // root.style.zIndex = 'var(--workbench-z-index-popover)';
