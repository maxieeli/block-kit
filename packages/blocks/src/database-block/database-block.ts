...
import { RangeManager } from '@maxiee/block-std';
import { Slot } from '@maxiee/block_global/utils';
...
import type { WorkbenchInnerModalWidget } from '../root-block/index.js';
import {
...
import { WORKBENCH_INNER_MODAL_WIDGET } from '../root-block/widgets/inner-modal/inner-modal.js';
...
@customElement('workbench-database')
...
> {
  override get topContenteditableElement() {
    if (this.rootElement instanceof EdgelessRootBlockComponent) {
      const note = this.closest<NoteBlockComponent>('workbench-note');
      return note;
    }
  ...
  get innerModalWidget() {
    return this.rootElement!.widgetElements[
      WORKBENCH_INNER_MODAL_WIDGET
    ] as WorkbenchInnerModalWidget;
  }

  static override styles = css`
    ${unsafeCSS(dataViewCommonStyle('workbench-database'))}
    workbench-database {
      display: block;
      border-radius: 8px;
  ...
  private renderTitle = (dataViewMethod: DataViewExpose) => {
    ...
    return html` <workbench-database-title
  ...
  override connectedCallback() {
    ...
    this.disposables.add(
      WorkbenchDragHandleWidget.registerOption({
...
declare global {
  interface HTMLElementTagNameMap {
    'workbench-database': DatabaseBlockComponent;
  }
}
