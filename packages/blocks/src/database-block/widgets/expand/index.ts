import type { EventName, UIEventHandler } from '@maxiee/block-std';
import { ShadowlessElement, WithDisposable } from '@maxiee/block-std';
import type { Disposable } from '@maxiee/block_global/utils';
import { assertExists, Slot } from '@maxiee/block_global/utils';
...
export function showDatabasePreviewModal(database: DatabaseBlockComponent) {
    ...
    return html`
      <style>
        ...
        .database-block-preview-close:hover {
          background-color: var(--workbench-hover-color-filled);
        }
      </style>
  ...
  modal.onclick = close;
  modal.style.backgroundColor = 'var(--workbench-black-60)';
...
export class ExpandDatabaseBlockModal extends WidgetBase {
  get database() {
    return this.closest('workbench-database');
  }
  ...
  expandDatabase = () => {
    const database = this.closest('workbench-database');
    if (database) {
...
export class DatabaseBlockModalPreview extends WithDisposable(
  ...
  protected override firstUpdated(_changedProperties: PropertyValues) {
    ...
    requestAnimationFrame(() => {
      this.querySelector('workbench-data-view-renderer')?.focusFirstCell();
    });
  }
