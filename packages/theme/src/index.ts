...
const themeToVar = (themeName: keyof WorkbenchTheme) =>
  `--workbench-${camelToKebab(themeName)}` as keyof WorkbenchCssVariables;
...
export type WorkbenchTheme = typeof lightTheme & {
  editorMode: 'page' | 'edgeless';
};

export type WorkbenchCssVariables = {
  [Key in `--workbench-${Kebab<keyof WorkbenchTheme>}`]: string;
};
...
export const baseTheme = {
  ...
  zoom: '1',
  scale: 'calc(1 / var(--workbench-zoom))',
...
} satisfies Omit<WorkbenchTheme, 'editorMode'>;
...
/**
 * Get workbench css variable name type safely
 * @param key in camel or kebab format
 * @param fallback
 * @returns
 */
export function cssVar(
  key: keyof WorkbenchCssVariables | keyof WorkbenchTheme,
  fallback?: string
) {
  const varName = key.startsWith('--')
    ? key
    : themeToVar(key as keyof WorkbenchTheme);
